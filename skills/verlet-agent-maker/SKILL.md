---
name: verlet-agent-maker
description: Author, validate, publish, and launch a Verlet agent manifest — the versioned declaration of what an agent is allowed to be, written in TOML and published through the local agent registry.
---
...
```text
intent
-> survey what exists        (verlet tool list / verlet agent list / verlet man)
-> draft manifest TOML       (this skill's reference below)
-> verlet agent plan        (offline compile: refs, hashes, counts)
-> fix until plan is clean
-> verlet agent publish     (fail-closed resolve; immutable record + alias)
-> start a thread on it      (console picker, or verlet agent run)
```
...
```sh
verlet agent init my-agent            # scaffold my-agent.verlet.agent.toml
$EDITOR my-agent.verlet.agent.toml
verlet agent plan my-agent.verlet.agent.toml      # offline; lists verified/unverified refs
verlet agent publish my-agent.verlet.agent.toml   # verifies op refs; prints ref, hashes, alias
verlet tool list                                  # read active operation hashes
verlet agent list                                  # confirm the record
verlet agent show agent://my-agent@0.1.0           # full record JSON
verlet agent run agent://my-agent@0.1.0 --input "hello"   # ephemeral one-shot
```

All registry commands take `--registry-root <path>`; the default agent
registry is `.verlet/agents` under the working directory. Agent `plan` and
`publish` also take `--operations-registry-root <path>`; the default
operations registry is `.verlet/operations`. A daemon-backed console picks
...
stop and build that first with the `verlet-tool-maker` skill — a manifest

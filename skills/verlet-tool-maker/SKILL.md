---
name: verlet-tool-maker
description: Create, inspect, build, or publish a Verlet tool package — a versioned black-box tool contract over the Wasm operation ABI, described by verlet.tool.toml and proven by fixtures.
---
...
The goal is a versioned black-box tool contract, not framework glue. Verlet
...
Build a deterministic utility, wrap it in the Verlet operation ABI, then
describe the accepted interface in `verlet.tool.toml`.

```text
source or artifact
-> Verlet Wasm operation ABI
-> verlet.tool.toml package manifest
-> tool build receipt
-> fixture proof
...
6. Declare runtime source in `verlet.tool.toml`:
...
9. Run `verlet tool build --package verlet.tool.toml`.
10. Fix schemas, capabilities, ABI exports, and fixtures until the build passes.
11. Publish with `verlet tool publish --package verlet.tool.toml`.
...
When `verlet` is installed on `PATH`, use:
...
V0 Rust tools compile to `wasm32-unknown-unknown` and expose the Verlet
operation ABI. The minimal crate shape is:

```toml
[package]
name = "verlet-wasm-my-tool"
version = "0.1.0"
edition = "2024"
publish = false
...
[dependencies]
verlet-guest-sdk = { path = "../path/to/verlet-guest-sdk" }
...
- `__verlet_describe_module__`, which writes an `OperationManifest`.
- `__verlet_call_operation__`, which routes an operation id to the function
  implementation.

Start from these repo fixtures instead of guessing the ABI:

- Pure JSON utility:
  `crates/verlet-kernel/tests/fixtures/wasm-csv-profile/`
- Internal HTTP utility using host imports:
  `crates/verlet-kernel/tests/fixtures/wasm-employee-lookup/`

For the HTTP pattern, use `verlet_guest_sdk::http_request` with an
`Invocation`, exact capability grants in `verlet.tool.toml`, and no ambient
network access.
...
Declare capabilities as requests. At build/publish Verlet reconciles them
...
Registry view, or `verlet man <record>` — never invent it.

## Validation Standard

A package is not done until all of these are true:

- `verlet tool build --package verlet.tool.toml` passes.
...
- `verlet tool publish --package verlet.tool.toml` writes a registry record.
...
- Treating MCP as the canonical contract. MCP is a surface/import shape; the
  Verlet tool interface is the durable contract.

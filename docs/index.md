...
- Use the checked-in Pi kit for pinned `read`, `write`, `edit`, `find`, and
  ...
  at install time. Models see a closed nested-argument schema while the host
  binds the canonical instance workspace root; raw virtual-bash operation
  aliases cannot bypass that split.

# Cooldis Kernel Threat Model

This document records security threats to the Cooldis runtime kernel and its
...
The lint in `crates/cooldis-kernel/tests/threat_model_lint.rs` enforces the entry
...
- Affected surface: `crates/cooldis-kernel/src/daemon/daemon_config.rs`, `crates/cooldis-kernel/src/daemon/daemon_io.rs`
...
- Status: OPEN
- Severity: High
- Threat: Any process that reaches the daemon Unix socket or loopback WebSocket can send `initialize` and invoke process, filesystem, provider-auth, approval, mandate, and binding methods. The console session token is enabled only when console assets are configured, and a pre-existing Unix socket directory is not tightened by the listener.
...
- Mitigation: Required: authenticate every control-plane transport, enforce owner-only Unix socket access including pre-existing parents, bind caller identity to the connection, and authorize privileged methods.
- Deterministic guard: None. Required: denied controls for unauthenticated TCP and Unix clients, socket-mode checks, and per-method authorization tests.
...
- Mitigation: Required: define the launcher as an explicit trust boundary, issue least-authority daemon credentials per projection, and require authentication in any non-stdio wrapper.
- Deterministic guard: None. Required: projection tests that prove the delegated method set and reject credentials or methods outside it.

## TM-INGRESS-004: Debug RPC inherits the control plane's transport weakness

- Status: OPEN
- Severity: Medium
- Threat: The debug client can call an arbitrary JSON-RPC method on a configurable WebSocket endpoint and carries no authentication material unless the endpoint independently requires it. Treating it as a harmless diagnostic can expose the full control plane.
- Affected surface: `crates/cooldis-kernel/src/cli/debug_rpc.rs`, `crates/cooldis-kernel/src/adapters/codex_tui.rs`
- Mitigation: Required: require explicit authenticated operator configuration, label the command as privileged, and consider a narrow debug method allowlist.
- Deterministic guard: None. Required: CLI tests that reject unauthenticated privileged endpoints once control-plane authentication exists.
...
- Affected surface: `crates/cooldis-kernel/src/kernel/admission.rs`, `crates/cooldis-kernel/src/adapters/app_server/connection.rs`, `crates/cooldis-kernel/src/daemon/daemon_io.rs`
- Mitigation: Required: authenticate ingress before admission, carry a stable principal and policy decision into the admission record, and reject a turn when either identity or authority is missing.
- Deterministic guard: None. Required: lifecycle tests proving unauthenticated and cross-principal submissions cannot append admission or turn records.
...
- Affected surface: `crates/cooldis-kernel/src/agent/manifest_bind.rs`, `crates/cooldis-agent/src/manifest_schema.rs`
...
- Mitigation: Required: require an authenticated operator capability for approval and mandate mutation, persist actor and policy provenance, and prevent self-approval by the executing agent principal.
- Deterministic guard: None. Required: denied-control tests for unprivileged, cross-tenant, and self-approval callers.
...
- Affected surface: `crates/cooldis-kernel/src/kernel/supervisor.rs`, `crates/cooldis-kernel/src/kernel/supervisor/tests.rs`, `crates/cooldis-history/src/lib.rs`, `crates/cooldis-history-sqlite/src/tests.rs`
...
- Deterministic guard: `crates/cooldis-kernel/src/kernel/supervisor/tests.rs` and `crates/cooldis-history-sqlite/src/tests.rs` pin unknown-tenant, scope-mismatch, and wrong-parent rejection paths.
...
- Affected surface: `crates/cooldis-process/src/execution.rs`, `crates/cooldis-process/src/live.rs`, `crates/cooldis-kernel/src/adapters/app_server/connection.rs`
...
- Affected surface: `crates/cooldis-kernel/src/daemon/remote_store/process_executor.rs`, `crates/cooldis-kernel/src/daemon/remote_store/placement.rs`, `crates/cooldis-kernel/src/daemon/remote_store/lease.rs`
...
- Affected surface: `crates/cooldis-process/src/live.rs`, `crates/cooldis-vbash/src/lib.rs`, `crates/cooldis-vbash/src/harness.rs`, `crates/cooldis-kernel/src/capabilities/execution.rs`
...
- Deterministic guard: `crates/cooldis-process/src/live.rs`, `crates/cooldis-vbash/src/lib.rs`, and `crates/cooldis-kernel/src/capabilities/execution/tests.rs` pin truncation, retention ceilings, and spill behavior.
...
- Affected surface: `crates/cooldis-process/src/execution.rs`, `crates/cooldis-process/src/live.rs`
...
- Deterministic guard: `crates/cooldis-process/src/execution.rs` and `crates/cooldis-process/src/live.rs` include process-backed tests for cancellation after the leader exits, partial output, and group cleanup.
...
- Affected surface: `crates/cooldis-kernel/src/cli/daemon.rs`, `crates/cooldis-kernel/src/adapters/app_server/mod.rs`, `crates/cooldis-kernel/src/kernel/supervisor.rs`
...
- Status: OPEN
- Severity: Medium
- Threat: When Cooldis is container PID 1, it has init responsibilities. Current reaping runs only after explicit cleanup of a known process group, so unrelated orphaned descendants or abrupt termination paths can accumulate zombies or escape the owned-group fold.
- Affected surface: `crates/cooldis-process/src/execution.rs`, `crates/cooldis-process/src/live.rs`, `crates/cooldis-kernel/src/daemon/daemon_config.rs`
- Mitigation: Required: document that a real init must wrap Cooldis, or implement and test complete PID 1 signal forwarding and child reaping.
- Deterministic guard: None. Required: a container smoke with Cooldis as PID 1 that exercises normal exit, cancellation, daemon termination, grandchildren, and orphan reaping.
...
- Affected surface: `crates/cooldis-history/src/lib.rs`, `crates/cooldis-history-sqlite/src/lib.rs`, `crates/cooldis-history-sqlite/src/tests.rs`
...
- Deterministic guard: `crates/cooldis-history-sqlite/src/tests.rs` runs the shared fenced-append conformance sequence against both stores and verifies conflict batches leave no mutation.
...
- Affected surface: `crates/cooldis-history/src/lib.rs`, `crates/cooldis-history-sqlite/src/lib.rs`, `crates/cooldis-kernel/src/daemon/remote_store/endpoint.rs`
...
- Affected surface: `crates/cooldis-history/src/lib.rs`, `crates/cooldis-history/src/tests.rs`, `crates/cooldis-history-sqlite/src/tests.rs`
...
- Deterministic guard: `crates/cooldis-history/src/tests.rs` and `crates/cooldis-history-sqlite/src/tests.rs` pin empty-provenance rejection and atomic invalid-payload rejection.
...
- Affected surface: `crates/cooldis-history-sqlite/src/lib.rs`, `crates/cooldis-kernel/src/daemon/daemon_io.rs`, `crates/cooldis-kernel/src/daemon/remote_store/queue.rs`, `crates/cooldis-metadata/src/lib.rs`
...
- Affected surface: `crates/cooldis-metadata/src/secret_store.rs`, `crates/cooldis-metadata/src/secret_store/tests.rs`, `crates/cooldis-wasm/src/runner.rs`, `docs/secret-management.md`
...
- Deterministic guard: `crates/cooldis-metadata/src/secret_store/tests.rs` and `crates/cooldis-wasm/src/runner.rs` pin ref validation, missing-secret failure, redacted status, and header injection checks.
...
- Affected surface: `crates/cooldis-metadata/src/secret_store.rs`, `crates/cooldis-metadata/src/provider_store.rs`
...
- Affected surface: `crates/cooldis-process/src/live.rs`, `crates/cooldis-process/src/execution.rs`, `crates/cooldis-kernel/src/daemon/remote_store/process_executor.rs`
...
- Affected surface: `crates/cooldis-kernel/src/daemon/daemon_io.rs`
...
- Mitigation: Required: cap concurrent connections, use bounded outbound queues with explicit slow-consumer behavior, reduce or budget message size, and add request deadlines.
...
- Affected surface: `crates/cooldis-process/src/live.rs`, `crates/cooldis-kernel/src/capabilities/execution.rs`, `crates/cooldis-kernel/src/agent/agent_process.rs`
...
- Deterministic guard: `crates/cooldis-process/src/live.rs` and `crates/cooldis-kernel/src/capabilities/execution/tests.rs` pin process-limit, timeout, cancellation, and output-cap behavior.
...
- Affected surface: `crates/cooldis-kernel/src/adapters/app_server/connection.rs`, `crates/cooldis-process/src/live.rs`

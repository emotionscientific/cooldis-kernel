...
| RPC thread methods | [RPC Control Plane](app-server.md) | app-server method docs | partial | `thread/spawn` remote placement and idempotent `thread/submit` use the store-hosted child queue when sync is served; thinking params, precedence, item stream projection, and broadcast-lag resynchronization are documented. Generated method-level manual/reference table remains follow-up work. |

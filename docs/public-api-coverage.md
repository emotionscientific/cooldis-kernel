...
```text
ABI operation contract -> lawful projections -> visible attachments -> host enforcement
```
...
| `verlet init`, `verlet agent init` | [Verlet Agent CLI](agent-cli.md) | `verlet init --help`, `verlet agent init --help` | partial | Folder-first project graph is the only output form; add generated synopsis/options/output/exit-status manual. |
...
| `verlet kit install`, `verlet kit list`, `verlet kit remove` | [Rust Wasm Operation Dev Kit](wasm-operation-dev-kit.md#kits) | `verlet kit --help` and subcommand help | partial | Local path install validates and publishes every member before atomically overwriting one installed-kit record; the checked-in Pi kit proves four records, five pinned default-manifest rows, package-declared MCP projection names, closed derived model schemas, a guest `/workspace` root backed by the instance's witnessed host mount, and no raw virtual-bash alias around a surface envelope. List supports human and JSON projections, remove deletes only the record, and generated man pages remain follow-up work. |
...
| `verlet tool publish` | [Tool Publish Storage](publish-storage.md), [Rust Wasm Operation Dev Kit](wasm-operation-dev-kit.md), [Verlet Tool Maker Skill](../skills/verlet-tool-maker/SKILL.md) | `verlet tool publish --help` | partial | Package publish persists the accepted interface; generate manual with registry writes, capability declarations, and replacement semantics. |
| `verlet tool run` | [ABI](abi.md), [Rust Wasm Operation Dev Kit](wasm-operation-dev-kit.md), [RPC Control Plane](app-server.md#secretresolve) | `verlet tool run --help` | partial | Published tools resolve declared secret refs through the connected instance before local Wasm execution; generate manual with stdin/stdout/stderr, mounts, JSON output, and exit taxonomy. |
...
| `verlet tool source` | [Verlet MCP Server](mcp-server.md), [Secret Management](secret-management.md) | `verlet tool source --help` | partial | Remote MCP source add/discover/list/show/remove uses the project instance through `mcpSource/*`; generate source-record and imported-tool manual pages with attachment config, discovery receipts, and failure taxonomy. |
...
| `verlet auth` | [Metadata And Provider Auth Storage](provider-storage.md), [Chat Console](chat.md), [Secret Management](secret-management.md) | `verlet auth --help`, `verlet auth login --help` | partial | Model-provider API-key set/status/delete and OpenAI Codex browser/device OAuth login route through the preferred project or user instance with redacted output; generated per-command manuals remain follow-up work. |
| `verlet secret` | [Secret Management](secret-management.md), [RPC Control Plane](app-server.md#secretlist) | `verlet secret --help` | partial | Import/set/list/status/delete route through the preferred project or user instance; import reads the client environment locally and public output remains redacted. Generate per-command manual pages and richer attachment receipt docs. |
| `verlet identity` | [RPC Control Plane](app-server.md#bootstrap-and-credentials) | `verlet identity --help` | partial | Bootstrap remains an offline first-operator operation; declare/mint/revoke/list route through the authenticated operator connection and do not accept caller-supplied actor ids. Generated per-command manuals remain follow-up work. |
...
| `verlet chat` | [Chat Console](chat.md), [Provider Setup](provider-setup.md), [RPC Control Plane](app-server.md), [Provider Adapter Surface](provider-adapters.md) | `verlet chat --help` | partial | Bundled full-screen terminal console discovers the project instance, shares bounded auto-start with other clients, and includes the `/setup` provider window, model picker, custom provider records, and OpenAI Codex login; richer generated manual remains follow-up work. |
| `verlet debug rpc` | [RPC Control Plane](app-server.md) | `verlet debug rpc --help` | partial | Protocol debug client discovers the current instance endpoint, supports Unix sockets and WebSockets, and preserves explicit URL/config precedence; formal man page remains follow-up work. |
| `verlet debug bind` | [RPC Control Plane](app-server.md), [CLI](cli.md) | `verlet debug bind --help` | partial | Receipt-only effective bind explanation discovers the live owner for `thread/events/list` or reads an offline Turso journal; formal man page remains follow-up work. |
| `verlet debug journal` | [RPC Control Plane](app-server.md#state-home-access-rules), [CLI](cli.md) | `verlet debug journal --help` | partial | Raw event record inspection discovers the owner RPC for live state and uses direct read-only Turso access for cold state; formal man page remains follow-up work. |
| `verlet serve` | [Verlet Daemon](daemon.md), [Verlet IO](io.md), [CLI](cli.md) | `verlet serve --help` | partial | Foreground serving, client auto-start, and idle shutdown are implemented; add a generated serve man page. |
...
| `verlet host run` | [RPC Control Plane](app-server.md#config-driven-multi-instance-host) | `verlet host run --help` | covered | Config-driven boot validates all managed instances before start, routes credential digests on one listener, starts an instance-owned mandate clock by default, exposes `/healthz`, and drains on SIGTERM/SIGINT. |
...
| Multi-tenant host facade | [Runtime Primitives](developers/runtime-primitives.md), [RPC Control Plane](app-server.md#config-driven-multi-instance-host) | `verlet host run --help`; Rust API module docs for `adapters::host` | covered | `VerletHost` and `verlet host run` provide config-driven one-listener credential-digest routing, isolated hosted instances with default mandate clocks, health checks, and explicit lifecycle control. |
| Instance endpoint record | [RPC Control Plane](app-server.md#endpoint-record-and-discovery) | `resolve_instance_endpoint` | covered | Every app-server instance publishes `endpoint.json` beside its state roots after its Unix socket is live, removes owned records on explicit shutdown, and exposes PID-checked discovery without port scanning. |
| Virtual-bash commands | [Command Contracts](command-contracts.md), [ABI](abi.md) | `man <command>` inside agent turns | partial | Live operation shell commands expose a thread-visible command contract with `verlet run` origin, IO, capability, exit-status fields, and retrievable `/spill` receipts for oversized bash and process streams. Surface-declared operations are withheld because their bound envelope must be assembled by the direct tool router. Richer package-authored docs still need to flow into the runtime registry. |
...
| RPC query/control methods | [RPC Control Plane](app-server.md) | app-server method docs, `verlet-workbench-smoke` | partial | Agent, operation, enriched model catalog, secret, identity, session-scoped `model/select`, thread event, coupling, `approval/resolve`, and waiting methods are implemented; value-carrying secret and mint methods are Unix-only and Host-authority effects are witnessed without parameters. Generated endpoint/manual pages remain follow-up work. |
| First-party thread tools | [Standard Operations](standard-operations.md), [Unified Orchestration Semantics](adr/0006-unified-orchestration-semantics.md) | Published `verlet-threads` operation manuals and virtual-bash commands | covered | Spawn defaults through the bound default-manifest alias; spawn, submit/steer, wait, status, and cancel address children by parent-scoped `task_name`, while raw ids remain in durable receipts and the journal. Unpinned pre-v0.3 package refs are rejected; pinned refs still resolve only against a preserved old-name record with the exact recorded hash. |
| Agent manifests | [Agent Manifest Ontology](agent-manifest-ontology.md), [Verlet Agent CLI](agent-cli.md), [V1 Release Candidate Gate](v1-release-candidate.md) | `verlet agent * --help` | partial | Persisted pre-rename records and grant-string payloads fail with republish guidance. The document is a preset for non-authority runtime payload and proposed bindings; opening `binding.attached` events establish recorded tool authority, and their fold is the only current toolset source. Receipt-only pre-binding streams require a new thread. Typed workspace binding, opt-in no-mount workspace skill discovery, finite-or-unlimited `max_tool_rounds`, and explicit per-tool attachment config are documented with durable bind witnesses; a generated schema reference remains follow-up work. |
...
| Provider adapters | [Provider Adapter Surface](provider-adapters.md), [Chat Console](chat.md) | config/help docs | partial | Generic protocol adapters plus the documented `openai-codex` integration are implemented; generated provider configuration reference remains follow-up work. |
| Secret references | [Secret Management](secret-management.md), [RPC Control Plane](app-server.md#secretresolve) | `verlet secret --help` | partial | The owning instance provides redacted management RPCs plus Unix-only resolution for local tool injection; provider-specific secret manager adapters and richer attachment receipts remain V2. |
| Identity/RBAC adapters | Identity/RBAC adapter boundary | n/a | reserved | V2 adapter boundary; V1 remains local principals, authority classes, and explicit attachments. |

...
| Embedded multi-tenant host facade | [Runtime Primitives](developers/runtime-primitives.md) | Rust API module docs for `adapters::host` | partial | `VerletHost` provides one-listener credential routing, isolated hosted instances, and explicit lifecycle control; a generated Rust API reference remains a documentation follow-up. |

...
| RPC thread methods | [RPC Control Plane](app-server.md) | app-server method docs | partial | Thread handle dispatch identity, thinking params, precedence, item stream projection, and broadcast-lag resynchronization are documented; generated method-level manual/reference table remains follow-up work. |
...
| Daemon config | [Cooldis Daemon](daemon.md), [Cooldis IO](io.md) | `cooldis daemon config validate --help` | partial | Placement defaults, store-primary `[daemon.sync]`, Telegram, and `clock.tick` route config are documented; add versioned config schema reference. |
| Daemon store-primary sync HTTP | [Cooldis Daemon](daemon.md), [ADR 0006](adr/0006-unified-orchestration-semantics.md) | `[daemon.sync]` config validation | covered | V1 push, verified-cursor pull, and lease renewal are projected over a loopback HTTP or Unix listener; the live WebSocket lane remains deliberately absent. |

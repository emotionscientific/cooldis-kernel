...
| `verlet kit install`, `verlet kit list`, `verlet kit remove` | [Rust Wasm Operation Dev Kit](wasm-operation-dev-kit.md#kits) | `verlet kit --help` and subcommand help | partial | Local path install validates and publishes every member before atomically overwriting one installed-kit record; list supports human and JSON projections, remove deletes only the record, and daemon restart synthesizes installed tools into the default manifest. Generated man pages remain follow-up work. |

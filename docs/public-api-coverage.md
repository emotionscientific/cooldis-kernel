...
| RPC orchestrator boundary | [RPC Control Plane](app-server.md), [ADR 0009](adr/0009-orchestrator-boundary-v0.md) | app-server method docs | covered | `ingress/submit` provides attributed, deduplicated, admission-gated envelope delivery; `stream/append` and `stream/read` provide witnessed client records with fenced atomic append and verified cursors. |

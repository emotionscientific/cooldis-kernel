...
| Agent manifests | [Agent Manifest Ontology](agent-manifest-ontology.md), [Verlet Agent CLI](agent-cli.md), [V1 Release Candidate Gate](v1-release-candidate.md) | `verlet agent * --help` | partial | New manifests require the `verlet.agent-manifest` kind; persisted pre-rename records remain readable. The document is a preset for non-authority runtime payload and proposed bindings; opening `binding.attached` events establish recorded tool authority, and their fold is the current toolset. Typed workspace binding, opt-in no-mount workspace skill discovery, finite-or-unlimited `max_tool_rounds`, and explicit per-tool attachment config are documented with durable bind witnesses; a generated schema reference remains follow-up work. |

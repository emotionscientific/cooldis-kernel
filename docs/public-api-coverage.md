...
| `verlet kit install`, `verlet kit list`, `verlet kit remove` | [Rust Wasm Operation Dev Kit](wasm-operation-dev-kit.md#kits) | `verlet kit --help` and subcommand help | partial | Local path install validates and publishes every member before atomically overwriting one installed-kit record; the checked-in Pi kit proves four records, five pinned default-manifest rows, and package-declared MCP projection names. List supports human and JSON projections, remove deletes only the record, and generated man pages remain follow-up work. |

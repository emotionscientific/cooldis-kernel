...
| `verlet chat` | [Chat Console](chat.md), [RPC Control Plane](app-server.md), [Provider Adapter Surface](provider-adapters.md) | `verlet chat --help` | partial | Bundled full-screen terminal console with a typed presentation/host split over app-server RPC, including an in-TUI `model/list`/`model/select` picker and the `openai-codex` ChatGPT-plan provider; richer generated manual remains follow-up work. |

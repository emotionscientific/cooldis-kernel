...
| `cooldis-messaging/message_react` | [Cooldis IO](io.md), [RPC Control Plane](app-server.md), [Agent Manifest Ontology](agent-manifest-ontology.md) | kernel package manual via operation registry | partial | Content-addressed reaction requests are implemented with `messaging.react`; add richer generated examples and route-specific adapter notes. |

...
| `cooldis man` | [Command Contracts](command-contracts.md), [V1 Release Candidate Gate](v1-release-candidate.md) | `cooldis man <published-tool> [operation] [--json]` | partial | Published package operation manuals are implemented; first-party CLI command manuals and imported MCP manuals remain follow-up work. |
...
| `cooldis rpc` | [RPC Control Plane And Dev Chat](app-server.md) | `cooldis rpc --help` | partial | Add generated endpoint/method manual and wire examples. |
| `cooldis dev chat` | [RPC Control Plane And Dev Chat](app-server.md), [Provider Adapter Surface](provider-adapters.md) | `cooldis dev chat --help` | partial | Separate developer help from public runtime command manual. |
| `cooldis dev rpc` | [RPC Control Plane And Dev Chat](app-server.md) | `cooldis dev rpc --help` | partial | Protocol debug client for running daemon WebSocket endpoints; formal man page remains follow-up work. |
| `cooldis dev tui` | [Operator Console](operator.md), [RPC Control Plane And Dev Chat](app-server.md) | `cooldis dev tui --help` | partial | Developer alias for `cooldis operator`; keep richer generated manual as follow-up work. |
...
| Command contracts | [Command Contracts](command-contracts.md) | `cooldis man`; issue [#104](https://github.com/emotionscientific/cooldis/issues/104) | partial | Tool-package operation manuals, generated fallback manuals, and JSON projection are implemented; generated first-party CLI command man pages remain. |
...
| RPC thread methods | [RPC Control Plane And Dev Chat](app-server.md) | app-server method docs | partial | Thread thinking params, precedence, and item stream projection are documented; generated method-level manual/reference table remains follow-up work. |
| RPC query/control methods | [RPC Control Plane And Dev Chat](app-server.md) | app-server method docs, `cooldis-workbench-smoke` | partial | Agent, operation, model, thread event, coupling, approval, and waiting query methods are implemented; `approval/resolve` witnesses abstract approval decisions. Generated endpoint/manual pages remain follow-up work. |
...
| Daemon config | [Cooldis Daemon](daemon.md), [Cooldis IO](io.md) | `cooldis daemon config validate --help` | partial | Add versioned config schema reference. |

...
| `verlet host run` | [RPC Control Plane](app-server.md#config-driven-multi-instance-host) | `verlet host run --help` | covered | Config-driven boot validates all managed instances before start, routes credential digests on one listener, exposes `/healthz`, and drains on SIGTERM/SIGINT. |
...
| Multi-tenant host facade | [Runtime Primitives](developers/runtime-primitives.md), [RPC Control Plane](app-server.md#config-driven-multi-instance-host) | `verlet host run --help`; Rust API module docs for `adapters::host` | covered | `VerletHost` and `verlet host run` provide config-driven one-listener credential-digest routing, isolated hosted instances, health checks, and explicit lifecycle control. |

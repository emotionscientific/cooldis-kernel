...
| `verlet debug rpc` | [RPC Control Plane](app-server.md) | `verlet debug rpc --help` | partial | Protocol debug client for running server WebSocket endpoints; formal man page remains follow-up work. |
| `verlet debug bind` | [RPC Control Plane](app-server.md), [CLI](cli.md) | `verlet debug bind --help` | partial | Receipt-only effective bind explanation over live `thread/events/list` or an offline Turso journal; formal man page remains follow-up work. |
| `verlet debug journal` | [RPC Control Plane](app-server.md#state-home-access-rules), [CLI](cli.md) | `verlet debug journal --help` | partial | Raw event record inspection uses owner RPC for live state and direct read-only Turso access for cold state; formal man page remains follow-up work. |

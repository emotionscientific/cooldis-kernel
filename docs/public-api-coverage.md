...
| `verlet tool source` | [Verlet MCP Server](mcp-server.md), [Secret Management](secret-management.md) | `verlet tool source --help` | partial | Remote MCP source add/list/show/remove is implemented; generate source-record and imported-tool manual pages with attachment config, discovery receipts, and failure taxonomy. |
...
| `verlet chat` | [Chat Console](chat.md), [Provider Setup](provider-setup.md), [RPC Control Plane](app-server.md), [Provider Adapter Surface](provider-adapters.md) | `verlet chat --help` | partial | Bundled full-screen terminal console with a typed presentation/host split over app-server RPC, including the `/setup` provider window over `modelProvider/catalog`, an in-TUI `model/list`/`model/select` picker, custom provider records, and the `openai-codex` ChatGPT-plan provider; richer generated manual remains follow-up work. |

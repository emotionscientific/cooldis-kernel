...
`cooldis-trace-ab` runs the same prompt against pinned Pi 0.70.2 and a running
Cooldis app server, normalizes both traces to `cooldis.trace.common/1` JSONL,
...
Start a Cooldis daemon whose provider and coding-agent manifest point at the
...
```sh
cargo run -p cooldis-trace-ab -- run \
  --prompt-file /tmp/task.txt \
  --workspace /absolute/path/to/seed-workspace \
  --output /tmp/trace-ab-run \
  --provider shared-provider \
  --model shared-model \
  --cooldis-agent-ref agent://coding/trace-ab@latest \
  --cooldis-url ws://127.0.0.1:49200/rpc \
  --max-tool-rounds 64
```

`--provider` and `--model` are passed unchanged to both Pi and Cooldis. Configure
Pi's model registry and the Cooldis agent profile so those names resolve to the
same endpoint. The runner rejects an existing non-empty output directory,
copies the seed into separate `pi-workspace` and `cooldis-workspace` trees, and
...
- `cooldis.export.json`, `cooldis.rpc.jsonl`, and `cooldis.common.jsonl`;
...
Cooldis export collected after a failed turn.

Pi is launched exactly as
`npx --yes @mariozechner/pi-coding-agent@0.70.2 --mode rpc`; the harness sends
LF-delimited `prompt` and `get_state` commands. Cooldis uses the existing
...
```sh
cargo run -p cooldis-trace-ab -- convert-pi \
  --input session.jsonl --output pi.common.jsonl

cargo run -p cooldis-trace-ab -- convert-cooldis \
  --input cooldis.export.json --output cooldis.common.jsonl

cargo run -p cooldis-trace-ab -- diff \
  --pi pi.common.jsonl --cooldis cooldis.common.jsonl \
  --output diff.txt
```
...
Cooldis event envelopes remain under `details`; Cooldis thread projection,
...
The Cooldis fixture follows the checked-in `cooldis.debug.thread_export/1`

//! Wasm guest module exposing the Pi-parity find and grep tools over the
//! Verlet operation ABI (`cooldis_0.1`). One module for both: they share
//! the deterministic gitignore-aware walk in `verlet-tool-core`, so they
//! ship as one semantic unit.
//!
...
pub extern "C" fn __verlet_describe_module__(sink: u32) -> i32 {
    let manifest = verlet_guest_sdk::OperationManifest::new(vec![
        verlet_guest_sdk::OperationDefinition::new(FIND_ID, "find")
            .json_input()
            .json_output(),
        verlet_guest_sdk::OperationDefinition::new(GREP_ID, "grep")
            .json_input()
            .json_output(),
    ]);

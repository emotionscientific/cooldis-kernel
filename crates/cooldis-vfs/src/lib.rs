...
#[async_trait]
pub trait CooldisVfsBackend: FileSystem {
...
#[async_trait]
impl CooldisVfsBackend for InMemoryFs {}

#[derive(Clone)]
pub struct ReadOnlyFileSystem {
    inner: Arc<dyn CooldisVfsBackend>,
}

impl ReadOnlyFileSystem {
    pub fn new(inner: Arc<dyn CooldisVfsBackend>) -> Self {
        Self { inner }
    }
}
...
#[async_trait]
impl CooldisVfsBackend for ReadOnlyFileSystem {
    async fn flush(&self) -> bashkit::Result<()> {
...
#[async_trait]
impl CooldisVfsBackend for HostFileSystem {}

pub struct CooldisVfs {
    root: Arc<dyn CooldisVfsBackend>,
    mounts: RwLock<BTreeMap<PathBuf, Arc<dyn CooldisVfsBackend>>>,
    journal: Mutex<Vec<VfsMutation>>,
}

impl CooldisVfs {
    pub fn new(root: Arc<dyn CooldisVfsBackend>) -> Self {
        Self {
            root,
    ...
    fn resolve(&self, path: &Path) -> (Arc<dyn CooldisVfsBackend>, PathBuf) {
...
#[async_trait]
impl FileSystemExt for CooldisVfs {
    fn usage(&self) -> FsUsage {
        self.root.usage()
    }
...
#[async_trait]
impl FileSystem for CooldisVfs {
    async fn read_file(&self, path: &Path) -> bashkit::Result<Vec<u8>> {
...
#[async_trait]
impl CooldisVfsBackend for ManagedObjectStoreFs {
    async fn flush(&self) -> bashkit::Result<()> {
        let ops = {
            self.dirty
                .lock()

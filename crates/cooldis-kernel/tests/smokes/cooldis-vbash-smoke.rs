use cooldis::{
    CooldisSupervisor, TenantRegistration, TenantRuntimeContext, ThreadEvent, ThreadStartRequest,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    supervisor
        .register_tenant(TenantRegistration {
            context: TenantRuntimeContext::local(
                "tenant-smoke",
                "/tmp/cooldis-vbash-runtime",
                "/tmp/cooldis-vbash-state",
            ),
    ...
    println!("cooldis vbash smoke ok: {}", output.trim());
    Ok(())
}

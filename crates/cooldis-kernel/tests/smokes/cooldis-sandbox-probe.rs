...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let private_network_probe_requested =
        std::env::var_os("COOLDIS_SANDBOX_PROBE_PRIVATE_NETWORK").is_some();
    ...
    if !libkrun_pkg_config_available && !libkrun_library_hint_present {
        unavailable_reasons
            .push("libkrun was not found via pkg-config or COOLDIS_LIBKRUN_PATH".to_string());
    }

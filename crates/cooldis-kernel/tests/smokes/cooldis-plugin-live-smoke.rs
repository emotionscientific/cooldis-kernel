...
const FINAL_MARKER: &str = "COOLDIS_PLUGIN_TOOL_OK";

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = SmokeConfig::load()?;
    let temp = std::env::temp_dir().join(format!("cooldis-plugin-live-{}", Uuid::now_v7()));
    ...
    println!(
        "cooldis plugin live smoke ok model={} tool={} text={}",
        config.model,
        TOOL_NAME,
...
impl SmokeConfig {
    fn load() -> Result<Self, Box<dyn std::error::Error>> {
        let env_file = std::env::var("COOLDIS_PLUGIN_LIVE_ENV_FILE")
            .or_else(|_| std::env::var("COOLDIS_BIFROST_ENV_FILE"))
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from(DEFAULT_ENV_FILE));
        let file_env = read_env_file_if_exists(&env_file)?;

        let base_url = env_or_file("COOLDIS_PLUGIN_LIVE_BASE_URL", &file_env)
            ...
            .ok_or("missing COOLDIS_PLUGIN_LIVE_BASE_URL, COOLDIS_BIFROST_URL, or OPENAI_API_KEY")?
            .trim_end_matches('/')
            .to_string();

        let api_key = env_or_file("COOLDIS_PLUGIN_LIVE_KEY", &file_env)
            ...
            .ok_or("missing COOLDIS_PLUGIN_LIVE_KEY, COOLDIS_BIFROST_KEY, or OPENAI_API_KEY")?;

        let model = env_or_file("COOLDIS_PLUGIN_LIVE_MODEL", &file_env)

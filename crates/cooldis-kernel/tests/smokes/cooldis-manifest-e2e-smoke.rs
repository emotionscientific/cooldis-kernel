...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::var("COOLDIS_MANIFEST_E2E_LIVE").ok().as_deref() == Some("1") {
...
async fn run_parent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "cooldis manifest e2e smoke ok root={} thread={} manifest_hash={} messages={} binds={} exported_events={} exported_receipts={}",
        root.display(),
        thread_id,
...
async fn run_researcher() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "cooldis researcher manifest e2e smoke ok root={} thread={} manifest_hash={} tools=json_query,file_read",
        root.display(),
        thread_id,
        record.manifest_hash
    );
...
async fn build_app(root: &Path) -> Result<CooldisAppServer, Box<dyn std::error::Error>> {
    let listen = AppServerListenAddr::WebSocket("127.0.0.1:0".parse()?);
    let mut config = CooldisAppServerConfig::local(listen.clone(), root.join("workspace"));
    config.runtime_home = root.join("runtime");
    config.state_home = root.join("state");
    config.agent_registry_root = root.join("agents");
    Ok(CooldisAppServer::new_local(config).await?)
}
...
) -> Result<CooldisAppServer, Box<dyn std::error::Error>> {
    let listen = AppServerListenAddr::WebSocket("127.0.0.1:0".parse()?);
...
fn publish_manifest(
    root: &Path,
    agent_registry_root: &Path,
) -> Result<cooldis::PublishedAgentRecord, Box<dyn std::error::Error>> {
    let manifest_path = root.join("manifest-e2e.cooldis.agent.toml");
    std::fs::write(
        &manifest_path,
        r#"
[agent]
...
fn publish_researcher_manifest(
    ...
    let template = std::fs::read_to_string(
        repo.join("examples/agents/researcher/researcher.cooldis.agent.toml.in"),
    )?;
...
fn publish_researcher_exa_manifest(
    ...
    let manifest_path = root.join("researcher-search.cooldis.agent.toml");
    let template = std::fs::read_to_string(
        repo.join("examples/agents/researcher/researcher.cooldis.agent.toml.in"),
    )?;
    let rendered = render_researcher_template(&template, standard_ops)?
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("cooldis-kernel manifest dir should be under crates/")
        .to_path_buf()
}
...
async fn run_researcher_live() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let listen = AppServerListenAddr::WebSocket("127.0.0.1:0".parse()?);
    let mut config_app = CooldisAppServerConfig::local(listen, &workspace)
        .with_openai_chat_completions(
            "openai_compatible",
            &config.base_url,
            &config.api_key,
    ...
    .await?;
    println!(
        "cooldis researcher manifest live smoke ok model={} root={} thread={}",
        config.model,
        root.display(),
        thread_id
    );

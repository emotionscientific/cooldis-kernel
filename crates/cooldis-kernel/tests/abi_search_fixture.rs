...
async fn search_style_http_operation_registers_and_invokes_through_registry() {
    ...
    let output = registry
        .invoke_bytes("search", "search", br#"{"query":"cooldis wasm"}"#.to_vec())
        .await
        .unwrap();
...
async fn search_style_http_operation_runs_through_shell_command() {
    ...
    let output = harness
        .execute(r#"command -v search && search '{"query":"cooldis wasm"}'"#)
        .await
        .unwrap();

    assert!(output.success(), "{output:?}");
    assert!(output.stdout.contains("search\n"));
    assert!(output.stdout.contains("Cooldis runtime"));
...
async fn published_search_operation_resolves_secret_store_and_invokes_through_agent_router() {
    ...
    let result = router
        .invoke_tool_call(
            "call_1",
            "search",
            serde_json::json!({"query":"cooldis wasm"}),
        )
        .await;
    ...
    assert!(matches!(
        ...
        CanonicalMessage::ToolResult {
            ...
            content,
            ..
        } if tool_result_text(&content).contains("Cooldis runtime")
    ));
    server.await.unwrap();
...
fn render_search_fixture(url: &str, http_grant: &str) -> String {
    ...
    .to_string();
    let body = br#"{"query":"cooldis wasm"}"#;
...
fn temp_dir(label: &str) -> PathBuf {
    let path = std::env::temp_dir().join(format!("cooldis-search-{label}-{}", Uuid::now_v7()));

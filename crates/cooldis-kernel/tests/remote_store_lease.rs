...
use cooldis::daemon::remote_store::endpoint::CooldisDaemonSyncConfig;
...
use cooldis::{
    CooldisError, DaemonClock, EventKind, EventSequence, EventStore, EventStreamId, NewEventRecord,
    SqliteSessionStore,
};
use cooldis_runtime_contracts::{DispatchId, ThreadCoordinates};
use cooldis_sqlite::{Db, DbConfig, TransactionBehavior, params};
...
async fn first_grant_overlap_and_racing_releases_fail_closed() {
    ...
    assert!(
        matches!(
            loser_error,
            CooldisError::History(ref message)
...
fn temp_db_path(test_name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("cooldis-{test_name}-{}.sqlite3", Uuid::now_v7()))
}

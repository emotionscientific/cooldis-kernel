...
fn kernel_source_obeys_lexicon_banned_words() {
    let root = repo_root();
    let source_root = root.join("crates/cooldis-kernel/src");
...
fn scanner_flags_new_banned_identifier() {
    ...
    let occurrences = scan_source("crates/cooldis-kernel/src/example.rs", source);
...
fn count_baseline_reports_shrunk_debt() {
    ...
    let baseline = BTreeMap::from([(
        (
            "crates/cooldis-kernel/src/example.rs".to_string(),
            "capsule".to_string(),
        ),
        2,
    )]);
...
fn count_baseline_reports_new_debt_with_line_and_hint() {
    ...
    assert!(message.contains("crates/cooldis-kernel/src/example.rs:1: capsule"));

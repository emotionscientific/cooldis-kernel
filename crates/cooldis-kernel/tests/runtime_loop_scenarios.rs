...
async fn provider_loop_exposes_bash_tool_with_structured_output() {
    let client = Arc::new(ScriptedProviderClient::with_responses(vec![
        response_tool_call("bash", json!({"command": "printf 'COOLDIS\\n'"})),
    ...
    assert_eq!(output["stdout"], "COOLDIS\n");
...
async fn streaming_scenario_emits_deltas_usage_and_model_completion() {
    ...
    .await
    .unwrap();
    let trace = collect_until_output(&mut events, "COOLDIS").await;
    ...
    assert_event_order(
        ...
        "first text delta",
        |event| matches!(event, RuntimeEventKind::TextDelta { text } if text == "COOL"),
    );
    assert_event_order(
        trace.runtime_events(),
        "first text delta",
...
async fn kernel_thread_router() -> AgentToolRouter {
    ...
    let mut registration = KernelOperationRegistration::new(
        cooldis::COOLDIS_THREADS_PACKAGE,
        package.manifest.clone(),
    )
    .with_capability_grants(package.capability_grants.clone());
    registration.metadata.insert(
        cooldis::OPERATION_METADATA_RUNTIME_KIND.to_string(),
        serde_json::Value::String(cooldis::KERNEL_RUNTIME_KIND.to_string()),
    );
...
fn request_texts(request: &cooldis::ProviderRequest) -> Vec<String> {

...
pub(super) async fn run_coupling(mut args: Vec<OsString>) -> CooldisResult<()> {
    if args.is_empty()
        || args
            .first()
...
pub(super) async fn coupling_init(args: Vec<OsString>) -> CooldisResult<()> {
...
pub(super) async fn coupling_run(args: Vec<OsString>) -> CooldisResult<()> {
...
pub(super) fn load_replay_coupling_set_file(path: &Path) -> CooldisResult<BoundCouplingSet> {
...
pub(super) fn load_replay_events_from_export(path: &Path) -> CooldisResult<Vec<EventRecord>> {
    let value = read_json_file(path)?;
    let streams = value
        .get("streams")
...
pub(super) fn event_record_from_export_value(value: Value) -> CooldisResult<EventRecord> {
...
pub(super) fn read_json_file(path: &Path) -> CooldisResult<Value> {
    let raw = fs::read_to_string(path)
        .map_err(|err| usage_error(format!("failed to read {}: {err}", path.display())))?;
...
pub(super) fn parse_pinned_operation_ref(reference: &str) -> CooldisResult<ParsedOperationRef> {
    let body = reference
        .strip_prefix("op://")
...
pub(super) fn parse_coupling_init_args(args: Vec<OsString>) -> CooldisResult<CouplingInitArgs> {
    let mut name = None;
...
pub(super) fn parse_coupling_run_args(args: Vec<OsString>) -> CooldisResult<CouplingRunArgs> {
...
pub(super) fn write_coupling_project(name: &str, root: &Path, force: bool) -> CooldisResult<()> {
...
pub(super) fn render_coupling_cargo_toml(name: &str) -> CooldisResult<String> {
...
pub(super) fn coupling_operation_name(name: &str) -> CooldisResult<String> {
...
pub(super) fn coupling_crate_name(name: &str) -> CooldisResult<String> {
...
pub(super) const COUPLING_LIB_RS_TEMPLATE: &str = r#"use cooldis_guest_sdk::prelude::*;
use serde_json::json;

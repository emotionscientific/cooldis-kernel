...
pub(super) async fn run_blob(mut args: Vec<OsString>) -> CooldisResult<()> {
    if args.is_empty()
        || args
            .first()
...
pub(super) async fn blob_publish(args: Vec<OsString>) -> CooldisResult<()> {
...
pub(super) fn parse_blob_publish_args(args: Vec<OsString>) -> CooldisResult<BlobPublishArgs> {
    let mut file = None;
    let mut name = None;

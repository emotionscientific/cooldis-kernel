...
use crate::{
    ...
    ThreadCheckpoint, ThreadCheckpointId, ThreadCoordinates, ThreadId, ThreadLifecycleRecord,
    ThreadLifecycleSink, ThreadScope, ThreadTopology, TurnInput, TurnSubmissionMode,
};
...
impl CooldisSupervisor {
    ...
    pub async fn register_tenant(&self, registration: TenantRegistration) -> CooldisResult<()> {
        let mut tenants = self.inner.tenants.write().await;
        let tenant_id = registration.context.tenant_id().to_string();
        if tenants.contains_key(&tenant_id) {
            return Err(CooldisError::TenantAlreadyExists(tenant_id));
        }
    ...
    async fn validate_related_thread_scope(
        ...
        let related = tenant
            ...
            .await
            .map_err(|_| CooldisError::RelatedThreadNotFound(related_thread_id))?;
        let actual_scope = related.context().coordinates.scope();
        if actual_scope != *requested_scope {
            return Err(CooldisError::RelatedThreadScopeMismatch {
    ...
    pub async fn shutdown_thread(&self, tenant_id: &str, thread_id: ThreadId) -> CooldisResult<()> {
        self.tenant(tenant_id)
            .await?
            .host
    ...
    pub async fn shutdown_thread_at(&self, coordinates: &ThreadCoordinates) -> CooldisResult<()> {
        self.get_thread_at(coordinates).await?;
        self.shutdown_thread(&coordinates.tenant_id, coordinates.thread_id)
            .await
    }

    pub async fn shutdown_tenant(&self, tenant_id: &str) -> CooldisResult<Vec<ThreadId>> {
        self.tenant(tenant_id).await?.host.shutdown_all().await
    }

    pub async fn shutdown_all(&self) -> CooldisResult<Vec<(String, Vec<ThreadId>)>> {
        let tenants = {
    ...
    async fn tenant(&self, tenant_id: &str) -> CooldisResult<Arc<TenantRuntime>> {
        self.inner
            .tenants
            .read()
            .await
            .get(tenant_id)
            .cloned()
            .ok_or_else(|| CooldisError::TenantNotFound(tenant_id.to_string()))
    }
...
impl Default for CooldisSupervisor {
    fn default() -> Self {
        Self::new()
    }
}

use crate::{
    AgentManifestBindReceipt, AgentManifestCouplingBinding, CooldisError, CooldisResult,
...
) -> CooldisResult<ToolCallDecision> {
...
pub async fn active_tool_controller_for_request<S: EventStore + ?Sized>(
    ...
    tool_name: &str,
) -> CooldisResult<Option<ToolControllerBinding>> {
...
pub async fn active_manifest_bind_receipt<S: EventStore + ?Sized>(
    store: &S,
    coordinates: &ThreadCoordinates,
) -> CooldisResult<Option<(EventRecordId, AgentManifestBindReceipt)>> {
    ...
    let receipt =
        serde_json::from_value::<AgentManifestBindReceipt>(event.payload).map_err(|err| {
            CooldisError::History(format!("manifest.bind.completed payload is invalid: {err}"))
        })?;
...
pub async fn list_pending_tool_call_suspensions<S: EventStore + ?Sized>(
    store: &S,
    coordinates: &ThreadCoordinates,
) -> CooldisResult<Vec<PendingToolCallSuspension>> {
    let control_events = store
        ...
        .filter(|event| event.kind == EventKind::ToolCallDecision)
    {
        let payload = serde_json::from_value::<ToolCallDecisionPayload>(event.payload.clone())
            .map_err(|err| {
                CooldisError::History(format!("tool.call.decision payload is invalid: {err}"))
            })?;
...
) -> CooldisResult<PlacementDecision> {
    let control_events = store
        .read_events(&control_stream_id(&request.coordinates), None)
        .await
        .map_err(|err| CooldisError::History(err.to_string()))?;
...
fn latest_matching_mandate(
    events: &[EventRecord],
    request: &TurnContinuationDecisionRequest,
) -> CooldisResult<Option<MandateStartedPayload>> {

use crate::{CooldisError, CooldisResult, OperationImportPlan};
use cooldis_abi::{
...
pub fn render_openapi_import_artifact(plan: &OperationImportPlan) -> CooldisResult<Vec<u8>> {
    ...
    let manifest = serde_json::to_vec(&manifest).map_err(|error| {
        CooldisError::RuntimeFactory(format!("failed to encode import manifest: {error}"))
    })?;
    ...
    for operation in &plan.operations {
        ...
        requests.push(serde_json::to_vec(&request).map_err(|error| {
            CooldisError::RuntimeFactory(format!("failed to encode import request plan: {error}"))
        })?);
    }
...
fn encode_module(
    ...
    let max_memory_pages = u32::try_from(cooldis_wasm::DEFAULT_MEMORY_LIMIT_BYTES / 65536)
        .map_err(|_| {
    ...
    encode_export("__cooldis_describe_module__", 0, 3, &mut exports);
    encode_export("__cooldis_call_operation__", 0, 4, &mut exports);
...
fn artifact_u32(label: &str, value: usize) -> CooldisResult<u32> {
...
fn artifact_add(label: &str, left: u32, right: u32) -> CooldisResult<u32> {
...
fn checked_align(value: u32, alignment: u32) -> CooldisResult<u32> {

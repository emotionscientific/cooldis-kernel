...
impl LocalAgentRegistry {
    ...
    pub fn publish_plan(&self, plan: AgentPublishPlan) -> CooldisResult<PublishedAgentRecord> {
    ...
    pub fn load_record(&self, name: &str) -> CooldisResult<PublishedAgentRecord> {
    ...
    pub fn load_ref(&self, agent_ref: &str) -> CooldisResult<PublishedAgentRecord> {
    ...
    pub fn load_ref_with_alias_receipt(
        &self,
        agent_ref: &str,
    ) -> CooldisResult<(PublishedAgentRecord, Option<AgentAliasResolutionReceipt>)> {
    ...
    pub fn list_records(&self) -> CooldisResult<Vec<PublishedAgentRecord>> {
    ...
    pub fn resolve_alias(
        ...
        name: &str,
        alias: &str,
    ) -> CooldisResult<(PublishedAgentRecord, AgentAliasResolutionReceipt)> {
        let name = validate_record_name(name)?;
    ...
    pub fn alias_record_path(&self, name: &str, alias: &str) -> CooldisResult<PathBuf> {
        let name = validate_record_name(name)?;
        let alias = validate_record_name(alias)?;
        Ok(self
            .root
    ...
    pub fn version_record_path(&self, name: &str, version: &str) -> CooldisResult<PathBuf> {
        let name = validate_record_name(name)?;
        validate_version(version)?;
        Ok(self
            .root
    ...
    fn write_record_atomically(&self, record: &PublishedAgentRecord) -> CooldisResult<()> {
        let path = self.record_path(&record.name)?;
        write_json_atomically(&path, format!("agent record {:?}", record.name), record)
    }

    fn write_version_record_atomically(&self, record: &PublishedAgentRecord) -> CooldisResult<()> {
    ...
    fn write_latest_alias_atomically(&self, record: &PublishedAgentRecord) -> CooldisResult<()> {
    ...
    fn write_alias_record_atomically(&self, alias: &AgentAliasRecord) -> CooldisResult<()> {
        alias.validate()?;
...
impl AgentPublishPlan {
    ...
    pub fn verify_operation_refs(&mut self, operation_registry_root: &Path) -> CooldisResult<()> {
    ...
    fn validate_resolved_refs_for_publish(&self) -> CooldisResult<()> {
...
impl AgentToolRef {
    fn validate(&self) -> CooldisResult<()> {
        validate_record_name(&self.name)?;
...
pub fn agent_manifest_source_from_schema(manifest: &AgentManifestSchema) -> CooldisResult<String> {
    let json = serde_json::to_value(manifest).map_err(|err| {
        CooldisError::RuntimeFactory(format!("failed to encode agent manifest schema: {err}"))
    })?;
    ...
    toml::to_string_pretty(&toml::Value::Table(root)).map_err(|err| {
        CooldisError::RuntimeFactory(format!("failed to encode agent manifest TOML: {err}"))
    })
}
...
fn json_to_toml_value(value: &JsonValue) -> CooldisResult<Option<toml::Value>> {
    match value {
...
impl AgentRecordRef {
    pub fn parse(value: &str) -> CooldisResult<Self> {
        ...
        let name_and_version = without_scheme
            .rsplit('/')
            .next()
            .ok_or_else(|| CooldisError::RuntimeFactory("empty agent ref".to_string()))?;
...
fn parse_agent_tool_refs(tools: &[AgentManifestTool]) -> CooldisResult<Vec<AgentToolRef>> {
...
fn validate_tool_ref(value: &str) -> CooldisResult<()> {
...
fn validate_label(label: &str, value: &str) -> CooldisResult<()> {
    if value.is_empty()
        || !value
            .bytes()
...
fn canonical_json_from_schema(value: &AgentManifestSchema) -> CooldisResult<JsonValue> {
...
fn value_sha256(value: &JsonValue) -> CooldisResult<String> {
    let bytes = serde_json::to_vec(value).map_err(|err| {
        CooldisError::RuntimeFactory(format!("failed to encode resolved agent manifest: {err}"))
    })?;

...
impl HookHandler for CommandHookHandler {
    ...
    async fn run(&self, request: HookRequest) -> CooldisResult<HookHandlerOutput> {
        ...
        let mut child = command
            .spawn()
            .map_err(|err| CooldisError::RuntimeExecution(format!("hook spawn failed: {err}")))?;
        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(input.as_bytes()).await.map_err(|err| {
                CooldisError::RuntimeExecution(format!("failed to write hook stdin: {err}"))
            })?;
        }
        let output = timeout(
        ...
        .await
        .map_err(|_| {
            CooldisError::RuntimeExecution(format!("hook timed out after {}ms", self.timeout_ms))
        })?
        .map_err(|err| CooldisError::RuntimeExecution(err.to_string()))?;
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
            return Err(CooldisError::RuntimeExecution(if stderr.is_empty() {
        ...
        serde_json::from_str(&stdout).map_err(|err| {
            CooldisError::RuntimeExecution(format!("failed to parse hook stdout: {err}"))
        })
    }
}

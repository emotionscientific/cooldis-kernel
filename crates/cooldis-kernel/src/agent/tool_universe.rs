...
//! without Cooldis's consent — attachment tier 3. The kernel therefore never
...
#[cfg(test)]
use cooldis_runtime_contracts::MAX_JSON_SCHEMA_SUBSET_DEPTH;
...
impl WitnessedToolContract {
    ...
    /// schema hash.
    pub fn witness(definition: &ToolDefinition) -> CooldisResult<Self> {
...
impl ToolUniverseDiscovery {
    ...
    pub fn filtered(&self, include_tools: &BTreeSet<String>) -> CooldisResult<Self> {
        let tools = self
            .tools
            .iter()
...
impl ToolUniverseSearchSurface {
    ...
    fn resolve_contract<'a>(
        ...
        tool_name: &str,
        universe: Option<&str>,
    ) -> CooldisResult<ResolvedUniverseTool<'a>> {
        let matches = self
            .universes
            .iter()
            .filter(|mounted| {
    ...
    fn resolve_pinned_contract<'a>(
        &'a self,
        tool_name: &str,
    ) -> CooldisResult<Option<ResolvedUniverseTool<'a>>> {
...
fn optional_string_arg(arguments: &Value, key: &str) -> CooldisResult<Option<String>> {
    match arguments.get(key) {
        Some(Value::String(value)) => Ok(Some(value.clone())),
        Some(_) => Err(CooldisError::RuntimeExecution(format!(
...
fn required_string_arg(arguments: &Value, key: &str) -> CooldisResult<String> {
    optional_string_arg(arguments, key)?.ok_or_else(|| {
        CooldisError::RuntimeExecution(format!("tool surface call requires {key:?}"))
    })
}
...
fn validation_error(tool_name: &str, err: JsonSchemaValidationError) -> CooldisError {
    CooldisError::RuntimeExecution(format!(

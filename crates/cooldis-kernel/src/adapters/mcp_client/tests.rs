...
async fn mcp_stdio_provider_imports_and_invokes_tool() {
    ...
    assert!(tools.iter().any(|tool| tool.name == "cooldis_mcp_echo"));
    ...
    assert!(matches!(
        ...
        CanonicalMessage::ToolResult {
            ...
            crate::CanonicalContent::Text { text, .. }
                if text.contains("COOLDIS_MCP_ECHO_OK hello from test")
        ))
    ));
}
...
for line in sys.stdin:
    ...
    if method == "initialize":
        ...
        print(json.dumps({"jsonrpc":"2.0","id":request["id"],"result":{"tools":[{"name":"cooldis_mcp_echo","description":"Echo a message through a real MCP stdio server.","inputSchema":{"type":"object","additionalProperties":False,"properties":{"message":{"type":"string"}},"required":["message"]}}]}}), flush=True)
    elif method == "tools/call":
        message = request.get("params", {}).get("arguments", {}).get("message", "")
        print(json.dumps({"jsonrpc":"2.0","id":request["id"],"result":{"content":[{"type":"text","text":"COOLDIS_MCP_ECHO_OK " + message}],"isError":False}}), flush=True)
    else:

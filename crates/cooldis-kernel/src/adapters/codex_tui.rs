use crate::{
    CooldisError, CooldisResult, JsonRpcError, JsonRpcMessage, JsonRpcNotification, JsonRpcRequest,
...
pub const CODEX_TUI_TEST_CLIENT_NAME: &str = "cooldis-codex-tui-test";
...
pub type CooldisOperatorClient<S> = CodexTuiTestClient<S>;
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    pub async fn account_read(&mut self) -> CooldisResult<Value> {
        self.request("account/read", json!({ "includeToken": false }))
            .await
    }

    pub async fn model_list(&mut self) -> CooldisResult<Value> {
        self.request("model/list", json!({})).await
    }

    pub async fn config_read(&mut self, include_layers: bool) -> CooldisResult<Value> {
        self.request(
            "config/read",
            json!({
    ...
    pub async fn thread_start(&mut self, params: Value) -> CooldisResult<CodexTuiThread> {
    ...
    pub async fn thread_fork(&mut self, thread_id: &str) -> CooldisResult<CodexTuiThread> {
        let result = self
            .request(
                "thread/fork",
                json!({
    ...
    pub async fn thread_name_set(&mut self, thread_id: &str, name: &str) -> CooldisResult<Value> {
        self.request(
            "thread/name/set",
            json!({
                "threadId": thread_id,
    ...
    pub async fn thread_compact_start(&mut self, thread_id: &str) -> CooldisResult<Value> {
        self.request(
    ...
    pub async fn thread_list(&mut self) -> CooldisResult<Value> {
        self.request("thread/list", json!({})).await
    }

    pub async fn loaded_thread_list(&mut self) -> CooldisResult<Value> {
        self.request("thread/loaded/list", json!({})).await
    }

    pub async fn thread_unsubscribe(&mut self, thread_id: &str) -> CooldisResult<Value> {
    ...
    pub async fn turn_interrupt(&mut self, thread_id: &str, turn_id: &str) -> CooldisResult<Value> {
        self.request(
            "turn/interrupt",
            json!({
                "threadId": thread_id,
    ...
    pub async fn run_prompt(
        ...
        prompt: &str,
        timeout: Duration,
    ) -> CooldisResult<CodexTuiCompletedTurn> {
    ...
    pub async fn request(&mut self, method: &str, params: Value) -> CooldisResult<Value> {
    ...
    pub async fn notify(&mut self, method: &str, params: Option<Value>) -> CooldisResult<()> {
        write_jsonrpc_message(
            &mut self.websocket,
    ...
    pub async fn next_event(&mut self) -> CooldisResult<CodexTuiEvent> {
    ...
    async fn read_event(&mut self) -> CooldisResult<CodexTuiEvent> {
        loop {
            let message = self
                .websocket
                .next()
    ...
    pub async fn close(&mut self) -> CooldisResult<()> {
        self.websocket
            .close(None)
            .await
...
async fn initialize_remote_connection<S>(
    ...
    endpoint: &str,
    config: CodexTuiConnectConfig,
) -> CooldisResult<(Value, VecDeque<CodexTuiEvent>)>
where
    S: AsyncRead + AsyncWrite + Unpin,
{
...
fn string_field(value: &Value, field: &str, context: &str) -> CooldisResult<String> {
    value
        .get(field)
        .and_then(Value::as_str)
...
fn thread_from_result(result: Value, method: &str) -> CooldisResult<CodexTuiThread> {
    let thread = result
        .get("thread")
        .cloned()

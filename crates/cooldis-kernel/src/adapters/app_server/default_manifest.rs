...
use super::CooldisAppServerConfig;
...
/// idempotently by content (D3): if `cooldis/default@latest` exists and its
...
fn load_existing_default_manifest(
    registry: &LocalAgentRegistry,
) -> CooldisResult<ExistingDefaultManifest> {
    if registry
...
impl DefaultManifestPublishLock {
    fn acquire(registry: &LocalAgentRegistry) -> CooldisResult<Self> {
...
fn synthesize_default_manifest_with_version(
    ...
    supports_streaming: bool,
    version: &str,
) -> CooldisResult<AgentManifestSchema> {
...
fn default_manifest_tools(
    config: &CooldisAppServerConfig,
) -> CooldisResult<Vec<AgentManifestTool>> {
    ...
    if let Some(registry_root) = bindings.registry_root.as_ref() {
        ...
        if let Ok(record) = registry.load_record(COOLDIS_THREADS_PACKAGE) {
    ...
    if bindings.load_all_active_when_unbound {
        for record in registry.list_records()? {
            if record.name == COOLDIS_PROCESS_PACKAGE {
                continue;
            }
            records.insert(record.name.clone(), record);
        }
...
fn default_manifest_publish_plan(
    ...
    supports_streaming: bool,
    version: &str,
) -> CooldisResult<AgentPublishPlan> {
...
fn default_manifest_source(manifest: &AgentManifestSchema) -> CooldisResult<String> {
    ...
    toml::to_string(&source).map_err(|err| {
        CooldisError::RuntimeFactory(format!("failed to encode default agent manifest: {err}"))
    })
}

fn ensure_default_record_identity(record: &PublishedAgentRecord) -> CooldisResult<()> {
...
fn patch_bump_version(version: &str) -> CooldisResult<String> {
...
fn invalid_default_version(version: &str) -> CooldisError {
    CooldisError::RuntimeFactory(format!(
        "default manifest latest version {version:?} is not a patch-bumpable semver"
    ))
}

fn absolute_path_string(path: &Path) -> CooldisResult<String> {

...
use crate::{
    ...
    WasmRuntimeArtifact, cooldis_messaging_kernel_package, cooldis_threads_kernel_package,
};
use cooldis_io_core::{ConversationKind, IoActor, IoConversation, IoSource, IoTarget};
...
fn runtime_factory_with_tool_router(
    client: Arc<dyn ProviderClient>,
    ...
    config.max_tokens = 128;
    Arc::new(
        CanonicalProviderRuntimeFactory::new(config, client).with_tool_router(Arc::new(router)),
    )
}
...
#[tokio::test]
async fn runtime_message_react_appends_requested_egress_for_unique_quote() {
    let router = kernel_messaging_router().await;
    let client = Arc::new(RecordingClient::with_responses(vec![
    ...
    let store = Arc::new(InMemorySessionStore::new());
    let host = RuntimeHost::with_session_store(
        runtime_factory_with_tool_router(provider_client, router),
        store.clone(),
    );
    let thread = host
        .start_thread(
        ...
        .await
        .unwrap();
    append_routed_ingress_artifacts(
        store.as_ref(),
    ...
    host.submit(
        thread.context().coordinates.thread_id,
        "turn-1",
        "Please remember blue apples.",
    )
    .await
    .unwrap();
    assert_output(&mut events, "done").await;

    let requested = requested_egress_events(store.as_ref(), &thread.context().coordinates).await;
    assert_eq!(requested.len(), 1);
    assert_eq!(
        requested[0].payload["requested_by_tool_call_id"].as_str(),
        Some("call_1|fc_1")
    );
    assert_eq!(
        requested[0].payload["egress_kind"]["type"].as_str(),
        Some("platform_action")
    );
    assert_eq!(
        requested[0].payload["egress_kind"]["action"].as_str(),
        Some("reaction")
    );
    assert_eq!(
        requested[0].payload["egress_kind"]["payload"]["message_id"].as_str(),
        Some("5551")
    );
    assert_eq!(
        requested[0].payload["egress_kind"]["payload"]["emoji"].as_str(),
        Some("👍")
    );
}

#[tokio::test]
async fn runtime_message_react_reports_ambiguous_quote_then_longer_quote_succeeds() {
    let router = kernel_messaging_router().await;
    let client = Arc::new(RecordingClient::with_responses(vec![
        response_text("ack one"),
        response_text("ack two"),
        response_tool_call_named(
            "message_react",
            serde_json::json!({
                "quote": "status",
    ...
    .await
    .unwrap();
    assert_output(&mut events, "ack one").await;
    append_routed_ingress_artifacts(
        store.as_ref(),
        &thread.context().coordinates,
        "turn-2",
    ...
    host.submit(
        thread.context().coordinates.thread_id,
        "turn-2",
        "second status unique",
    )
    .await
    .unwrap();
    assert_output(&mut events, "ack two").await;
    ...
    let messages = text_messages(&thread.session_context().await.unwrap().messages);
    assert!(
        messages
            .iter()
            .any(|text| text.contains("multiple messages matching quote"))
    );
    assert!(
        messages
            .iter()
            .any(|text| text.contains("first status unique"))
    );
    assert!(
        messages
            .iter()
            .any(|text| text.contains("second status unique"))
    );
}

#[tokio::test]
async fn runtime_message_react_reports_ambiguous_when_missing_id_candidate_also_matches() {
    let router = kernel_messaging_router().await;
    let client = Arc::new(RecordingClient::with_responses(vec![
        response_text("ack one"),
        response_text("ack two"),
    ...
    let thread = host
        ...
        .unwrap();
    let mut events = thread.subscribe_events();
    append_routed_ingress_artifacts_without_message_id(
        store.as_ref(),
        &thread.context().coordinates,
        "turn-1",
    )
    ...
    assert!(
        requested_egress_events(store.as_ref(), &thread.context().coordinates)
            .await
            .is_empty()
    );
    let messages = text_messages(&thread.session_context().await.unwrap().messages);
    ...
    assert!(
        messages
            .iter()
            .any(|text| text.contains("missing_external_message_id"))
    );
}

#[tokio::test]
async fn runtime_message_react_fails_closed_for_coalesced_multi_source_turn() {
    let router = kernel_messaging_router().await;
    let client = Arc::new(RecordingClient::with_responses(vec![
        response_tool_call_named(
            "message_react",
            serde_json::json!({
                "quote": "second coalesced",
                "emoji": "👍"
            }),
        ),
        response_text("done"),
    ...
    let thread = host
        ...
        .await
        .unwrap();
    append_coalesced_routed_ingress_artifacts(
        store.as_ref(),
        &thread.context().coordinates,
        "turn-1",
        &["5551", "5552"],
    ...
    assert!(
        messages
            .iter()
            .any(|text| text.contains("coalesced_multi_source_turn"))
    );
}

#[tokio::test]
async fn runtime_message_react_no_match_returns_tool_error_without_event() {
    let router = kernel_messaging_router().await;
    let client = Arc::new(RecordingClient::with_responses(vec![
        response_tool_call_named(
            "message_react",
    ...
    assert!(
        messages
            .iter()
            .any(|text| text.contains("no message matching quote"))
    );
}

#[tokio::test]
async fn runtime_message_react_zero_width_only_quote_does_not_match_hidden_text() {
    let router = kernel_messaging_router().await;
    let client = Arc::new(RecordingClient::with_responses(vec![
        response_tool_call_named(
            "message_react",
...
#[tokio::test]
async fn runtime_message_react_requires_messaging_react_grant() {
    let registry = kernel_messaging_registry().await;
    let router = kernel_messaging_router_with_grants(registry, BTreeSet::new());
    let client = Arc::new(RecordingClient::with_responses(vec![
        response_tool_call_named(
            "message_react",
            serde_json::json!({
    ...
    assert!(
        messages
            .iter()
            .any(|text| text.contains("missing capability grants: messaging.react"))
    );
}
...
async fn kernel_messaging_registry() -> Arc<OperationRegistry> {
    let registry = Arc::new(OperationRegistry::new());
    let package = cooldis_messaging_kernel_package();
    let mut registration = KernelOperationRegistration::new(
        crate::COOLDIS_MESSAGING_PACKAGE,
...
async fn kernel_messaging_router() -> AgentToolRouter {
    let registry = kernel_messaging_registry().await;
    let package = cooldis_messaging_kernel_package();
    kernel_messaging_router_with_grants(registry, package.capability_grants)
}

fn kernel_messaging_router_with_grants(
    registry: Arc<OperationRegistry>,
    ...
    AgentToolRouter::new(registry)
        ...
        .with_tool_aliases(vec![OperationToolAlias {
            tool_name: crate::MESSAGE_REACT_OPERATION.to_string(),
            registered_name: crate::COOLDIS_MESSAGING_PACKAGE.to_string(),
            operation_name: crate::MESSAGE_REACT_OPERATION.to_string(),
        }])
}

async fn append_routed_ingress_artifacts(
    store: &InMemorySessionStore,
    coordinates: &ThreadCoordinates,
    turn_id: &str,
    external_message_id: &str,
) {
    append_routed_ingress_artifacts_with_options(
        store,
        coordinates,
        turn_id,
...
async fn append_routed_ingress_artifacts_without_message_id(
    store: &InMemorySessionStore,
    coordinates: &ThreadCoordinates,
    turn_id: &str,
) {
...
async fn append_coalesced_routed_ingress_artifacts(
    store: &InMemorySessionStore,
    coordinates: &ThreadCoordinates,
    turn_id: &str,
    external_message_ids: &[&str],
) {
    let mut metadata = BTreeMap::new();
    metadata.insert("cooldis_coalesced".to_string(), "true".to_string());
    metadata.insert(
        "cooldis_coalesced_batch_size".to_string(),
        external_message_ids.len().to_string(),
    );
    let source_ids = external_message_ids
        .iter()
        .map(|message_id| Some(*message_id))
        .collect::<Vec<_>>();
    append_routed_ingress_artifacts_with_options(
        store,
        coordinates,
        turn_id,
        &source_ids,
        external_message_ids.first().copied(),
        metadata,
    )
    .await;
}

async fn append_routed_ingress_artifacts_with_options(
    store: &InMemorySessionStore,
    coordinates: &ThreadCoordinates,
    turn_id: &str,
    source_external_message_ids: &[Option<&str>],
    thread_external_message_id: Option<&str>,
    ingress_metadata: BTreeMap<String, String>,
) {
    let source = IoSource::new("telegram.bot", "main");
    let conversation = IoConversation::new("telegram:chat:123", ConversationKind::Direct);
    let actor = IoActor::new("telegram:user:42");
    let mut source_events = Vec::new();
    for (index, external_message_id) in source_external_message_ids.iter().enumerate() {
        let source_key = if source_external_message_ids.len() == 1 {
            turn_id.to_string()
        } else {
            format!("{turn_id}-source-{}", index + 1)
        };
        let mut source_payload = serde_json::json!({
            "schema": EventKind::IoIngressReceived.payload_schema_id(),
            "route_id": "main",
            "dedupe_key": format!("telegram.bot:main:update:{source_key}"),
            "external_conversation_id": conversation.external_conversation_id.clone(),
            "external_actor_id": actor.external_actor_id.clone(),
            "envelope_digest": format!("sha256:{source_key}"),
        });
        if let Some(external_message_id) = external_message_id {
            source_payload["external_message_id"] = serde_json::json!(external_message_id);
        }
        let source_event = store
            ...
            .next()
            .unwrap();
        source_events.push(source_event);
    }
        ...
        .append_events(
            ...
            vec![NewEventRecord::discharged(
                ...
                serde_json::json!({
                    ...
                    "route_id": "main",
                    "policy_hash": "sha256:test-admission-policy",
                    "decision": "queue",
                    "source_ingress_event_ids": source_event_ids,
                }),
                EventProvenance {
                    source_streams: vec![crate::control_stream_id(coordinates)],
                    source_event_ids: source_events.iter().map(|event| event.id).collect(),
                    discharged_by: Some("policy:admission_route:main".to_string()),
                    function: Some("admission_route/v1".to_string()),
                    config_hash: Some("sha256:test-admission-policy".to_string()),
                    ..EventProvenance::default()
                },
            )],
        )
    ...
    let mut thread_payload = serde_json::json!({
        "schema": EventKind::IoIngressReceived.payload_schema_id(),
        "route_id": "main",
        "dedupe_key": format!("telegram.bot:main:update:{turn_id}"),
        "external_conversation_id": target.conversation.external_conversation_id,
        "external_actor_id": "telegram:user:42",
        "envelope_digest": format!("sha256:{turn_id}"),
        "turn_id": turn_id,
        "source_scope": "telegram.bot:main",
        "ingress_envelope_id": format!("ingress-{turn_id}"),
        "target": target_value,
        "ingress_metadata": ingress_metadata,
    });
    if let Some(external_message_id) = thread_external_message_id {
        thread_payload["external_message_id"] = serde_json::json!(external_message_id);
        thread_payload["ingress_metadata"]["telegram_message_id"] =
...
async fn requested_egress_events(
    store: &InMemorySessionStore,
    coordinates: &ThreadCoordinates,
    ...
    store
        ...
        .unwrap()
        .into_iter()
        .filter(|event| event.kind == EventKind::IoEgressRequested)
        .collect()
}

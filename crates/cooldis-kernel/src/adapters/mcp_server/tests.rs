...
async fn mcp_server_lists_tools_without_daemon_connection() {
    ...
    let config = CooldisMcpServerConfig {
    ...
    assert_eq!(init["result"]["serverInfo"]["name"], "cooldis-mcp-server");
    ...
    assert!(
        tools["result"]["tools"]
            ...
            .unwrap()
            .iter()
            .any(|tool| tool["name"] == "cooldis_prompt")
    );
...
async fn mcp_server_runs_prompt_and_command_through_daemon() {
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"cooldis_thread_start","arguments":{}}}"#,
        )
        .await;
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"cooldis_command_exec","arguments":{"command":["/bin/sh","-lc","printf MCP_EXEC_OK"]}}}"#,
        )
        .await;
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":6,"method":"tools/call","params":{"name":"cooldis_prompt","arguments":{"message":"this should not reach the provider","thread":{"model":"not-declared"}}}}"#,
        )
        .await;
...
async fn mcp_prompt_lets_model_shaped_agent_see_and_call_search_shell_command() {
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cooldis_prompt","arguments":{"message":"Use bash to call search for the query cooldis. If it fails because a credential is missing, report the failure."}}}"#,
        )
        .await;
    let prompt = read_json_response(&mut lines, 2).await;
    assert_eq!(
...
async fn mcp_capsule_binding_tools_update_global_scope() {
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cooldis_capsule_binding_set","arguments":{"scope":{"kind":"global"},"operationName":"search"}}}"#,
        )
        .await;
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"cooldis_capsule_binding_list","arguments":{"scope":{"kind":"global"}}}}"#,
        )
        .await;
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"cooldis_capsule_binding_resolve","arguments":{}}}"#,
        )
        .await;
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":6,"method":"tools/call","params":{"name":"cooldis_capsule_binding_delete","arguments":{"scope":{"kind":"global"},"operationName":"search"}}}"#,
        )
        .await;
...
async fn mcp_prompt_lets_model_shaped_agent_call_secret_backed_search_wasm() {
    ...
    let (base_url, http_server) = spawn_http_server(
        r#"{"results":[{"title":"Cooldis runtime","url":"https://cooldis.local"}]}"#,
        vec![
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cooldis_prompt","arguments":{"message":"Use bash to call search for the query cooldis."}}}"#,
        )
        .await;
    let prompt = read_json_response(&mut lines, 2).await;
    assert_eq!(
...
async fn mcp_prompt_rejects_thread_capsule_bindings() {
    ...
    send(
            &mut write,
            r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cooldis_prompt","arguments":{"message":"Use bash to call search for the query cooldis. If it fails because a credential is missing, report the failure.","thread":{"capsuleBindings":{"operationNames":["search"]}}}}}"#,
        )
        .await;
    let prompt = read_json_response(&mut lines, 2).await;
    assert_eq!(
        ...
        if !has_tool_result {
            ...
            return Ok(ProviderResponse {
                content: vec![CanonicalContent::tool_call(
                    ...
                    "bash",
                    json!({
                        "command": "command -v search && search '{\"query\":\"cooldis\"}'"
                    }),
                )],

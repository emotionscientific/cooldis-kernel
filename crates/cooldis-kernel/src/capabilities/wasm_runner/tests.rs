use super::*;
use crate::{
    CooldisProcessBackend, ExecutionPrincipal, Principal, RuntimeHost, RustWasmBuildOptions,
    ThreadCoordinates, ThreadTopology, WasmOperationMode, build_rust_wasm_module,
};
...
fn echo_guest() -> &'static str {
    r#"
        (module
          (import "cooldis" "input_read" (func $input_read (param i32 i32) (result i32)))
          (import "cooldis" "output_write" (func $output_write (param i32 i32)))
          (import "cooldis" "log" (func $log (param i32 i32)))
...
fn capability_manifest_guest() -> String {
    let manifest = serde_json::json!({
        ...
        "operations": [{
            ...
            "events": "none",
            "mode": "sync",
            "required_capabilities": ["cooldis:secret/read"]
        }]
    })
    .to_string();
    format!(
...
async fn wasm_cat_vfs() -> Arc<CooldisVfs> {
...
async fn wasm_operation_artifact_validation_rejects_missing_manifest() {
    ...
    assert!(err.contains("__cooldis_describe_module__"), "{err}");
}
...
async fn wasm_operation_artifact_validation_rejects_unknown_import() {
    let guest = r#"
        (module
          ...
          (memory (export "memory") 1)
          (func (export "__cooldis_describe_module__") (param i32) (result i32)
            i32.const 0)
          (func (export "__cooldis_call_operation__") (param i32 i32 i32 i32 i32) (result i32)
            i32.const 0))
        "#;
...
async fn wasm_operation_can_be_invoked_as_process_handle() {
    ...
    assert_eq!(process.backend(), &CooldisProcessBackend::WasmOperation);
...
async fn wasm_operation_accepts_invocation_context_grants_before_guest_execution() {
    let context = InvocationContext::anonymous().with_grant("cooldis:secret/read");
    let factory = WasmRuntimeFactory::new(
        WasmRuntimeConfig::new(WasmRuntimeArtifact::bytes(wat_guest(
            operation_guest_with_required_capabilities(vec!["cooldis:secret/read"]),
        )))
...
#[tokio::test]
async fn wasm_cat_reads_file_from_cooldis_vfs() {
    let factory = WasmRuntimeFactory::new(
...
#[tokio::test]
async fn wasm_tail_reads_last_two_lines_from_cooldis_vfs() {
    let factory = WasmRuntimeFactory::new(
        WasmRuntimeConfig::new(WasmRuntimeArtifact::bytes(wasm_vfs_tools_guest()))
            .with_vfs(wasm_cat_vfs().await),
    )
...
async fn wasm_http_operation_can_call_mock_exa_api() {
    ...
    let output = factory
        .invoke_operation_bytes("search", br#"{"query":"cooldis"}"#.to_vec())
        .await
        .unwrap();

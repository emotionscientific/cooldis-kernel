...
use crate::{
    CanonicalContent, CanonicalMessage, CooldisAppServer, CooldisAppServerConfig,
    CooldisDaemonIoBridge, CooldisError, CooldisResult, EventKind, EventStore, EventStreamId,
    SessionEntryKind, SessionStore, SqliteSessionStore, SystemDaemonClock, ThreadContext,
    ThreadCoordinates, ThreadId, ThreadStartRequest, ThreadStatus, ThreadTerminalState, TurnInput,
};
...
impl ProcessRemoteThreadExecutor {
    ...
    async fn spawn_shielded(&self, request: RemoteThreadSpawnRequest) -> CooldisResult<()> {
        ...
        tokio::spawn(async move {
            ...
            if let Err(error) = &outcome {
                ...
                {
                    eprintln!(
                        "cooldis remote child bootstrap failure settlement failed: {settlement_error}"
                    );
                }
            }
            outcome
    ...
    fn state(&self, thread_id: ThreadId) -> CooldisResult<Arc<RemoteChildState>> {
        self.inner
            .states
            .read()
...
impl ProcessRemoteThreadExecutorInner {
    async fn spawn(self: Arc<Self>, request: RemoteThreadSpawnRequest) -> CooldisResult<()> {
        ...
        let process_task = tokio::spawn(async move {
            ...
            {
                eprintln!("cooldis remote child process death settlement failed: {error}");
            }
        });
...
impl RemoteThreadExecutor for ProcessRemoteThreadExecutor {
    ...
    async fn spawn(&self, request: RemoteThreadSpawnRequest) -> CooldisResult<()> {
        self.spawn_shielded(request).await
    }

    async fn submit(&self, request: RemoteThreadSubmitRequest) -> CooldisResult<ThreadStatus> {
    ...
    async fn observe(&self, thread_id: ThreadId) -> CooldisResult<RemoteThreadObservation> {
    ...
    ) -> CooldisResult<RemoteThreadWaitObservation> {
        let state = self.state(thread_id)?;
    ...
    loop {
        match tail.poll(&cursor).await {
            Ok(page) => {
                ...
                for record in page.records {
                        ...
                        .await
                    {
                        ...
                        Err(error) => {
                            eprintln!(
                                "cooldis remote tail failed to fold terminal record: {error}"
                            );
                            page_folded = false;
                            break;
                        }
            ...
            Err(error) => {
                ...
                eprintln!("cooldis remote tail poll failed for {stream_id}: {error}");
            }
        }
...
pub(crate) async fn run_remote_child(
    mut app_config: CooldisAppServerConfig,
...
async fn init_child_cursor_schema(store: SqliteSessionStore) -> CooldisResult<()> {
...
async fn load_child_cursor(
    store: &SqliteSessionStore,
    thread_id: ThreadId,
) -> CooldisResult<Option<crate::StreamCursorV1>> {
    ...
    rows.next()
        ...
        .map(|row| {
            ...
            serde_json::from_str(&encoded)
                .map_err(|error| CooldisError::History(format!("decode child cursor: {error}")))
        })
        .transpose()
}
...
async fn persist_child_cursor(
    ...
    let encoded = serde_json::to_string(cursor)
        .map_err(|error| CooldisError::History(format!("encode child cursor: {error}")))?;
...
fn remote_error(message: impl Into<String>) -> CooldisError {
...
mod tests {
    ...
    fn request_fixture() -> RemoteThreadSpawnRequest {
        ...
        RemoteThreadSpawnRequest {
            ...
            dispatch_id: cooldis_runtime_contracts::DispatchId::new("spawn-dispatch"),
    ...
    async fn detached_bootstrap_failure_settles_after_the_caller_is_cancelled() {
        ...
        let authority = Arc::new(
            SqliteStreamLeaseAuthority::new(
                store.clone(),
                super::super::endpoint::CooldisDaemonSyncConfig::default(),
                Arc::new(SystemDaemonClock),
            )
            .await
            .unwrap(),
        ...
        let child_root = std::env::temp_dir().join(format!(
            "cooldis-remote-cancelled-bootstrap-{}",
            uuid::Uuid::now_v7()
        ));
        let executor = ProcessRemoteThreadExecutor::new(
            ...
            None,
            child_root.clone(),
            child_root.join("missing-cooldis-executable"),
        )
        .await
        .unwrap();
        let request = request_fixture();

...
impl SqliteSyncEndpoint {
    ...
    pub async fn rejection_witnesses(
    ...
    ) -> CooldisResult<Vec<SyncPushRejectionWitnessV1>> {
        ...
        while let Some(row) = rows.next().await.map_err(storage_error)? {
            ...
            witnesses.push(serde_json::from_str(&encoded).map_err(|error| {
                CooldisError::History(format!("decode sync rejection witness: {error}"))
            })?);
        }
        Ok(witnesses)
    }

    async fn init_witness_schema(&self) -> CooldisResult<()> {
    ...
    async fn persist_rejection_witness(
        ...
        let witness_json = serde_json::to_string(&witness).map_err(|error| {
            CooldisError::History(format!("encode sync rejection witness: {error}"))
        })?;
        let store = self.store.clone();
        cancellation_safe(async move {
            let database = store.sqlite_database();
...
fn rejection_fingerprint(
    ...
    let encoded = serde_json::to_vec(&(request, reason)).map_err(|error| {
        CooldisError::History(format!("encode sync rejection fingerprint: {error}"))
    })?;
...
fn not_authorized() -> CooldisError {
    CooldisError::History("sync pull not authorized".to_string())
}
    ...
    T: Send + 'static,
{
    tokio::spawn(future).await.map_err(|error| {
        CooldisError::History(format!("sync endpoint transaction task failed: {error}"))
    })?
}

...
use crate::{
    CooldisError, CooldisResult, EventOrigin, EventProvenance, EventRecordId, EventSequence,
    EventStreamId, STREAM_RECORD_SCHEMA_V1, SqliteSessionStore, StreamCursorV1,
    StreamRecordEnvelopeV1, ThreadCoordinates, ThreadId,
};
...
impl SqliteRemoteIngressQueue {
    ...
    async fn init_schema(&self) -> CooldisResult<()> {
        ...
        cancellation_safe(async move {
            ...
            transaction
                .execute_batch(
                    r#"
                    CREATE TABLE IF NOT EXISTS cooldis_remote_ingress_queue (
                    ...
                    CREATE INDEX IF NOT EXISTS idx_cooldis_remote_ingress_queue_pending
                        ON cooldis_remote_ingress_queue(
                            target_thread_id,
                            acknowledged_at_ms,
                            sequence
                        );
    ...
    pub(crate) async fn pull_after(
        ...
        let target_thread_id = remote_ingress_queue_target(stream_id).ok_or_else(|| {
            CooldisError::History(format!("invalid remote ingress queue stream {stream_id}"))
        })?;
    ...
    async fn verify_cursor(
        ...
        let mut rows = connection
            .query(
                "SELECT event_id FROM cooldis_remote_ingress_queue
    ...
    async fn pending_rows(
        ...
        target_thread_id: ThreadId,
        after: Option<EventSequence>,
    ) -> CooldisResult<Vec<StoredQueueRow>> {
        let database = self.store.sqlite_database();
        let connection = database.connect().await.map_err(storage_error)?;
        ...
        let mut rows = connection
            .query(
                "SELECT sequence, event_id, entry_json
                 FROM cooldis_remote_ingress_queue
...
impl StoredQueueRow {
    fn into_wire(self, stream_id: EventStreamId) -> CooldisResult<StreamRecordEnvelopeV1> {
        let payload = serde_json::to_value(&self.entry).map_err(|error| {
            CooldisError::History(format!("encode remote ingress queue entry: {error}"))
        })?;
...
impl RemoteIngressQueue for SqliteRemoteIngressQueue {
    async fn enqueue(
        ...
        cancellation_safe(async move {
            ...
            let mut existing_rows = transaction
                .query(
                    "SELECT entry_json FROM cooldis_remote_ingress_queue
            ...
            transaction
                .execute(
                    "INSERT INTO cooldis_remote_ingress_queue (
    ...
    async fn acknowledge(
        ...
        cancellation_safe(async move {
            ...
            transaction
                .execute(
                    "UPDATE cooldis_remote_ingress_queue
...
fn validate_entry(entry: &RemoteIngressQueueEntryV1) -> CooldisResult<()> {
...
fn decode_entry(encoded: &str) -> CooldisResult<RemoteIngressQueueEntryV1> {
    let entry = serde_json::from_str::<RemoteIngressQueueEntryV1>(encoded).map_err(|error| {
        CooldisError::History(format!("decode remote ingress queue entry: {error}"))
    })?;
    validate_entry(&entry)?;
    Ok(entry)
}

fn parse_event_id(value: &str) -> CooldisResult<EventRecordId> {
    uuid::Uuid::parse_str(value)
        .map(EventRecordId::from_uuid)
        .map_err(|error| CooldisError::History(format!("decode queue event id: {error}")))
}
...
fn history_error(error: impl std::fmt::Display) -> CooldisError {

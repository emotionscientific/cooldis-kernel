...
use cooldis_runtime_contracts::{ThreadCoordinates, ThreadId};
...
pub type OperationEventStream = BoxStream<'static, CooldisProcessResult<OperationEvent>>;

#[async_trait]
pub trait CapabilityBridge: Send + Sync + 'static {
    async fn capabilities(&self) -> CooldisProcessResult<BridgeCapabilities>;

    async fn open_session(
        &self,
        request: OpenBridgeSessionRequest,
    ) -> CooldisProcessResult<BridgeSession>;

    async fn invoke(&self, request: OperationRequest)
    -> CooldisProcessResult<OperationEventStream>;

    async fn cancel(&self, operation_id: OperationId) -> CooldisProcessResult<()>;

    async fn close_session(&self, bridge_session_id: BridgeSessionId) -> CooldisProcessResult<()>;
}
...
impl CapabilityBridge for RejectingCapabilityBridge {
    ...
    async fn cancel(&self, _operation_id: OperationId) -> CooldisProcessResult<()> {
        Ok(())
    }

    async fn close_session(&self, _bridge_session_id: BridgeSessionId) -> CooldisProcessResult<()> {
        Ok(())
    }
}

...
fn virtual_process_retains_output_and_exit_status() {
    let process = VerletProcessHandle::from_virtual_command(
        "echo hi",
...
fn external_timeout_maps_to_terminal_process_state() {
    ...
    let process = VerletProcessHandle::from_external_command(
        &request,
...
async fn bridge_stream_is_live_and_retained() {
    ...
    let process = VerletProcessHandle::new(VerletProcessBackend::Bridge, "bridge exec");
    ...
    assert!(matches!(
        started.kind,
        VerletProcessEventKind::Started { .. }
    ));
    let stdout = live.recv().await.unwrap();
    assert!(matches!(
        stdout.kind,
        VerletProcessEventKind::Stdout { ref bytes } if bytes == b"hello"
    ));
    ...
    assert!(matches!(
        output.terminal,
        Some(VerletProcessTerminalState::Cancelled { reason }) if reason == "stop"
    ));
...
async fn async_manager_writes_to_stdin_capable_host_command() {
    ...
    assert!(terminated.snapshot.events.iter().any(|event| {
        matches!(
            &event.kind,
            VerletProcessEventKind::Cancelled { reason } if reason == "test complete"
        )
    }));
    assert!(manager.acknowledge_terminal(process_id).await.unwrap());
}

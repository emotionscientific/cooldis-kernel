use crate::{RegisteredOperation, VerletOperationsError, VerletResult};
...
impl OperationRegistry {
    ...
    ) -> VerletResult<RegisteredOperation> {
    ...
    async fn register_wasm(
        ...
        let mut manifest = match manifest {
            ...
            None => factory.describe().await?.ok_or_else(|| {
                VerletOperationsError::RuntimeFactory(format!(
    ...
    pub async fn set_kernel_dispatcher(
        ...
        match &entry.runtime {
            ...
            OperationRegistryEntryRuntime::Wasm { .. } => {
                Err(VerletOperationsError::RuntimeFactory(format!(
...
fn normalize_registration_name(name: &str) -> VerletResult<String> {
    let name = name.trim();
    if name.is_empty() {
        return Err(VerletOperationsError::RuntimeFactory(
...
pub fn filter_manifest_operations(
...
) -> VerletResult<WasmOperationManifest> {
    ...
    if !missing.is_empty() {
        ...
        return Err(VerletOperationsError::RuntimeFactory(format!(

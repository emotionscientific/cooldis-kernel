...
async fn cancelling_schema_task_finishes_and_releases_the_write_lock() {
    let db_path = temp_db_path("verlet-metadata-cancelled-schema-task");
    remove_sqlite_files(&db_path);
    let db = verlet_sqlite::Db::open(&db_path, verlet_sqlite::DbConfig::default())
        .await
        .unwrap();
    db.connect()
        .await
...

    {
        ...
        conn.execute(
            "UPDATE thread_lifecycle_records
             ...
             WHERE thread_id = ?1",
            verlet_sqlite::params![coordinates.thread_id.to_string()],
        )
        .await
        .unwrap();
    }
...
async fn turso_decodes_rusqlite_created_metadata_v1_fixture() {
    let db_path = temp_db_path("verlet-rusqlite-decode-compat");
    remove_sqlite_files(&db_path);

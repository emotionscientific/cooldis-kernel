...
use uuid::Uuid;
use verlet::kernel::history::ThreadBranchSelectedPayload;
use verlet::{
    ...
    CanonicalMessage, CanonicalStopReason, CouplingActivation, CouplingBudgetSpent,
    CouplingDischarge, CouplingExecutor, CouplingInvocation, CouplingRole, CouplingRunReceipt,
    CouplingRunStatus, CouplingSourceCut, CouplingSourceCutEntry, CouplingTemplateMaturity,
    DEBUG_THREAD_EXPORT_SCHEMA_V1, EventKind, EventOrigin, EventProvenance, EventRecord,
    EventRecordId, EventSequence, EventStreamId, FileDeltaKind, HookEventName, HookHandlerOutput,
    HookRequest, HookRunRecord, HookRunStatus, IngressOutcomeIntent, IngressSettledBy,
    IoIngressClaimedPayload, IoIngressSettledPayload, ObservationSourceRange,
    OpenAIChatCompletionsAdapter, OpenAIResponsesAdapter, OperationEvent, OperationExitStatus,
    OperationId, OperationLogLevel, OperationProjectionSet, ProviderApi, ProviderRequest,
    ProviderWireAdapter, RegisteredOperation, RuntimeApprovalDecision, RuntimeEventId,
    ...
    TurnContextSnapshot, UnixExecPayload, UnixExecutionMode, VerletProcessArtifact,
    VerletProcessExitStatus, VerletProcessFileDelta, VerletProcessOutput,
    VerletProcessTerminalState, WasmOperationDefinition, WasmOperationEventKind,
    WasmOperationManifest, WasmOperationMode, WasmOperationValueKind, coupling_template_catalog_v1,
...
fn hook_contracts_match_fixture() {
    ...
    let value = json!({
        "pre_tool_request": HookRequest::PreToolUse(verlet::PreToolUseHookRequest {
            turn_context: snapshot.clone(),
            call_id: "call_1".to_string(),
            tool_name: "echo_search".to_string(),
            arguments: json!({"input":"original"}),
        }),
        "post_tool_request": HookRequest::PostToolUse(verlet::PostToolUseHookRequest {
...
fn abi_and_process_contracts_match_fixture() {
    ...
    let output = VerletProcessOutput {
        stdout: b"{\"ok\":true}\n".to_vec(),
        stderr: b"{\"level\":\"info\"}\n".to_vec(),
        terminal: Some(VerletProcessTerminalState::Completed {
            status: VerletProcessExitStatus::success(),
        }),
        stdout_truncated: false,
        stderr_truncated: false,
        artifacts: vec![VerletProcessArtifact {
            artifact_id: "artifact_1".to_string(),
            path: Some(PathBuf::from("/workspace/out.json")),
            mime_type: Some("application/json".to_string()),
        }],
        file_deltas: vec![VerletProcessFileDelta {
    ...
    let mut unix_exec = UnixExecPayload::new("verlet run search search", "/workspace");
        ...
        messages: vec![
            ...
            CanonicalMessage::assistant(
                ...
                vec![
                    ...
                    CanonicalContent::tool_call("call_1|fc_1", "search", json!({"query":"verlet"})),
                ],
...
fn turn_snapshot() -> TurnContextSnapshot {
    TurnContextSnapshot {
        ...
        metadata: BTreeMap::from([("source".to_string(), "contract".to_string())]),
        environment: BTreeMap::from([("VERLET_TEST".to_string(), "1".to_string())]),

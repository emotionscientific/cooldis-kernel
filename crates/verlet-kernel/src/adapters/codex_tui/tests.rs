use super::*;
use crate::{AppServerListenAddr, VerletAppServer, VerletAppServerConfig};
use uuid::Uuid;
...
fn rpc_client_errors_render_without_the_runtime_factory_prefix() {
    assert_eq!(
        tui_error("failed to connect to the Verlet RPC endpoint `ws://127.0.0.1:1/rpc`")
            .to_string(),
        "failed to connect to the Verlet RPC endpoint `ws://127.0.0.1:1/rpc`"
    );
    assert_eq!(
        VerletError::RuntimeFactory("fixture failure".to_string()).to_string(),
...
async fn codex_tui_operator_client_covers_thread_lifecycle_methods() {
    ...
    let mut client = VerletOperatorClient::connect_unix(&socket, CodexTuiConnectConfig::default())
        .await
        .unwrap();
...
#[cfg(unix)]
async fn wait_for_socket(path: &PathBuf) -> VerletResult<()> {
    for _ in 0..1_500 {
        if path.exists() {
            return Ok(());
        }

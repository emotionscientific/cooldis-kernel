...
impl VerletAcpAgent {
    ...
    async fn handle_notification(&mut self, method: &str, params: serde_json::Value) {
        ...
        {
            eprintln!(
                "verlet-acp-agent: session/cancel notification failed: {}",
                err.message
            );
        }
    }
    ...
    async fn session_new(
            ...
            && !mcp_servers.is_empty()
        {
            return Err(AcpError::protocol(
                -32602,
                "ACP session/new mcpServers are not supported yet; configure MCP sources through Verlet manifest binding",
            ));
        }
        ...
        #[cfg(not(unix))]
        {
            ...
            return Err(AcpError::internal(
                "Verlet ACP daemon socket transport requires Unix",
            ));
        }
        #[cfg(unix)]
        {
            ...
            let thread = client.thread_start(thread_params).await.map_err(|err| {
                AcpError::internal(format!(
                    "Verlet thread/start failed for ACP session/new: {err}"
                ))
            })?;
            let model_list = client.model_list().await.map_err(|err| {
                AcpError::internal(format!(
                    "Verlet model/list failed for ACP session/new: {err}"
                ))
            })?;
        ...
        #[cfg(unix)]
        {
            ...
            let turn =
                turn_start_text_with_config(&mut client, &thread_id, &prompt_text, &session_config)
                    ...
                    .map_err(|err| {
                        AcpError::internal(format!(
                            "Verlet turn/start failed for ACP session/prompt: {err}"
                        ))
                    })?;
            ...
            tokio::spawn(async move {
                    ...
                    .await
                {
                    ...
                    Err(err) => vec![error_response(
                        request_id,
                        -32000,
                        format!("Verlet turn wait failed for ACP session/prompt: {err}"),
                        None,
                    )],
                };
    ...
    for block in prompt {
        match block.get("type").and_then(serde_json::Value::as_str) {
            ...
            Some("resource") => {
                return Err(AcpError::protocol(
                    -32602,
                    "ACP embedded resource content requires embeddedContext capability, which verlet-acp-agent does not advertise yet",
                ));
            }
            Some("image") => {
                return Err(AcpError::protocol(
                    -32602,
                    "ACP image content requires image capability, which verlet-acp-agent does not advertise yet",
                ));
            }
            Some("audio") => {
                return Err(AcpError::protocol(
                    -32602,
                    "ACP audio content requires audio capability, which verlet-acp-agent does not advertise yet",
                ));
            }
            Some(other) => {
                return Err(AcpError::protocol(
    ...
    fn from_model_list(model_list: &serde_json::Value) -> Self {
        ...
        if options.is_empty() {
            options.push(AcpModelOption {
                ...
                description: Some("Configured Verlet model".to_string()),
            });
        }
        Self {
...
mod tests {
    ...
    async fn acp_agent_session_prompt_projects_text_to_verlet_turn() {
        ...
        assert_eq!(
            response["result"]["verlet"]["assistantText"],
            "local:hello acp"
        );
        assert_eq!(
            response["result"]["verlet"]["threadId"],
            new_session["result"]["sessionId"]
        );
        assert!(
            response["result"]["verlet"]["turnId"]
                .as_str()
                .is_some_and(|turn_id| !turn_id.is_empty()),
            "{response}"
        );
        assert_eq!(
            response["result"]["verlet"]["turn"]["status"], "completed",
            "{response}"
        );
        assert!(
            response["result"]["verlet"]["turn"]["completedAt"].is_number(),
            "{response}"
        );
    ...
    async fn run_openai_compatible_acp_prompt_attempt(
        ...
        let live_result: crate::live_smoke_support::LiveSmokeResult<()> = async {
            ...
            let assistant_text = response["result"]["verlet"]["assistantText"]
                .as_str()
                .unwrap_or_default();
            ...
            println!(
                "verlet acp openai-compatible live ok model={} thread={} marker={} response={}",
                live_config.model,
                session_id,
                marker,
                compact_for_assertion(assistant_text)

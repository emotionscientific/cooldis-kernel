...
pub(super) async fn run_secret(mut args: Vec<OsString>) -> VerletResult<()> {
    if args.is_empty()
        || args
            .first()
...
pub(super) async fn secret_import(args: Vec<OsString>) -> VerletResult<()> {
...
pub(super) async fn secret_set(args: Vec<OsString>) -> VerletResult<()> {
...
pub(super) async fn secret_list(args: Vec<OsString>) -> VerletResult<()> {
...
pub(super) async fn secret_status(args: Vec<OsString>) -> VerletResult<()> {
    ...
    println!(
        ...
        serde_json::to_string_pretty(&status).map_err(|err| {
            VerletError::RuntimeFactory(format!("failed to encode secret status: {err}"))
        })?
    );
    Ok(())
}

pub(super) async fn secret_delete(args: Vec<OsString>) -> VerletResult<()> {
...
pub(super) fn parse_secret_import_args(args: Vec<OsString>) -> VerletResult<SecretImportArgs> {
    let mut name = None;
...
pub(super) fn parse_secret_set_args(args: Vec<OsString>) -> VerletResult<SecretSetArgs> {
    let mut name = None;
...
pub(super) fn default_project_state_home() -> PathBuf {
    let legacy = PathBuf::from(concat!(".", "cool", "dis/state"));
    if Path::new(".verlet").exists() || !legacy.exists() {
        PathBuf::from(".verlet/state")
    } else {
        eprintln!(
...
pub(super) fn default_user_state_home() -> VerletResult<PathBuf> {
    Ok(default_user_verlet_home()?.join("state"))
}
...
pub(super) async fn open_secret_store(
    state_home: Option<PathBuf>,
) -> VerletResult<SqliteSecretStore> {
...
pub(super) async fn open_provider_store(
    state_home: Option<PathBuf>,
) -> VerletResult<SqliteMetadataStore> {
...
pub(super) fn cross_process_database_guidance(alternative: &str) -> VerletError {
    usage_error(format!(
...
pub(super) fn secret_cli_error(err: impl std::fmt::Display) -> VerletError {
    VerletError::RuntimeFactory(format!("secret store failed: {err}"))
}

pub(super) fn provider_cli_error(err: impl std::fmt::Display) -> VerletError {
    VerletError::RuntimeFactory(format!("provider store failed: {err}"))
}

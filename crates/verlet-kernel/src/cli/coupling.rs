...
pub(super) async fn run_coupling(mut args: Vec<OsString>) -> VerletResult<()> {
    if args.is_empty()
        || args
            .first()
...
pub(super) async fn coupling_init(args: Vec<OsString>) -> VerletResult<()> {
...
pub(super) async fn coupling_run(args: Vec<OsString>) -> VerletResult<()> {
...
pub(super) fn load_replay_coupling_set_file(path: &Path) -> VerletResult<BoundCouplingSet> {
...
pub(super) async fn resolve_replay_artifact(
    ...
    let root = registry_root.unwrap_or_else(|| {
        std::env::temp_dir()
            .join(format!("verlet-coupling-replay-{}", Uuid::now_v7()))
            .join("operations")
    });
    let record = LocalOperationRegistry::new(&root)
...
pub(super) fn load_replay_events_from_export(path: &Path) -> VerletResult<Vec<EventRecord>> {
    let value = read_json_file(path)?;
    let streams = value
        .get("streams")
...
pub(super) fn event_record_from_export_value(value: Value) -> VerletResult<EventRecord> {
...
pub(super) fn read_json_file(path: &Path) -> VerletResult<Value> {
    let raw = fs::read_to_string(path)
        .map_err(|err| usage_error(format!("failed to read {}: {err}", path.display())))?;
...
pub(super) fn parse_pinned_operation_ref(reference: &str) -> VerletResult<ParsedOperationRef> {
    let body = reference
        .strip_prefix("op://")
...
pub(super) fn coupling_run_status_name(status: CouplingRunStatus) -> &'static str {
    match status {
...
pub(super) fn parse_coupling_init_args(args: Vec<OsString>) -> VerletResult<CouplingInitArgs> {
    let mut name = None;
...
pub(super) fn parse_coupling_run_args(args: Vec<OsString>) -> VerletResult<CouplingRunArgs> {
...
pub(super) fn write_coupling_project(name: &str, root: &Path, force: bool) -> VerletResult<()> {
...
pub(super) fn render_coupling_cargo_toml(name: &str) -> VerletResult<String> {
...
pub(super) fn coupling_operation_name(name: &str) -> VerletResult<String> {
...
pub(super) fn coupling_crate_name(name: &str) -> VerletResult<String> {
...
pub(super) fn guest_sdk_dependency_path() -> PathBuf {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../verlet-guest-sdk");

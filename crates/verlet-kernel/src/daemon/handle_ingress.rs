...
use crate::{
    CanonicalContent, CanonicalMessage, EventKind, EventRecord, EventStore, SessionContext,
    SessionEntryKind, SessionStore, SqliteSessionStore, ThreadCoordinates, ThreadJoinedPayload,
    ThreadTerminalState, VerletError, VerletResult,
};
...
impl ThreadHandleIngressAdapter {
    ...
    pub(crate) async fn enqueue_ready_once(&self) -> VerletResult<usize> {
...
impl ThreadTerminalSettlement {
    fn ingress_envelope(&self, context: &SessionContext) -> VerletResult<IngressEnvelope> {
...
fn fold_terminal_settlements(
    consumer: &ThreadCoordinates,
    events: &[EventRecord],
) -> VerletResult<Vec<ThreadTerminalSettlement>> {

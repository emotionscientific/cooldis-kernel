...
use crate::{
    EventKind, EventRecord, EventStore, EventStreamId, IoIngressReceivedPayload,
    SqliteSessionStore, ThreadCoordinates, ThreadJoinedPayload, ThreadTerminalState, VerletError,
    VerletResult, control_stream_id,
};
use std::collections::{BTreeMap, BTreeSet};
use verlet_io_core::IngressContent;
...
impl StartupRecoverySweep {
    ...
    pub(crate) async fn run_once(&self) -> VerletResult<StartupRecoveryReceipt> {
...
fn fold_joined_spawn_ids(events: &[EventRecord]) -> VerletResult<BTreeSet<String>> {
    events
        .iter()
...
) -> VerletResult<Option<ChildTerminalTruth>> {
...
fn fold_orphaned_process_dispatches(
    consumer: &ThreadCoordinates,
    events: &[EventRecord],
) -> VerletResult<Vec<HandleDispatchEnvelope>> {
...
fn history_error(err: impl std::fmt::Display) -> VerletError {
...
mod tests {
    ...
    use crate::{
        ...
        EventKind, EventProvenance, EventStore, EventStreamId, IoIngressReceivedPayload,
        NewEventRecord, ProviderApi, ProviderClient, ProviderRequest, ProviderResponse,
        ProviderResult, RuntimeHost, RuntimeStore, ThreadCoordinates, ThreadId,
        ThreadJoinedPayload, ThreadSpawnRequestedPayload, ThreadSpawnedPayload,
        ThreadTerminalState, ThreadTopology, VerletAppServer, VerletAppServerConfig,
        control_stream_id,
    };
    use serde_json::json;
    ...
    use verlet_io_core::{IngressContent, IngressEnvelope, IoDedupeKey};
    use verlet_process::{
        AsyncExecutionManager, AsyncProcessStartRequest, ExecutionDeadline, LiveProcessBackend,
        LiveProcessSpawn, LiveProcessStartRequest, VerletProcessBackend, VerletProcessEventKind,
        VerletProcessHandle,
    };
    use verlet_runtime_contracts::{
    ...
    async fn app_server_construction_runs_recovery_before_local_surfaces_are_available() {
        ...
        let mut config = VerletAppServerConfig::local(listen, &root);
        ...
        let _app = VerletAppServer::new_local(config).await.unwrap();

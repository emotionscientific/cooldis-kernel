...
impl StartupRecoverySweep {
    ...
    pub(crate) async fn run_once(
        &self,
    ) -> crate::kernel::runtime_host::VerletResult<StartupRecoveryReceipt> {
    ...
    async fn recover_threads(
        ...
        for binding in bindings {
            ...
            if binding.handle.kind != verlet_runtime_contracts::handle::HandleKind::Thread {
                eprintln!(
            ...
            let child_thread_id = match verlet_runtime_contracts::ThreadId::parse_str(
                &binding.handle.id,
            ) {
            ...
            let thread_stream = verlet_history::EventStreamId::for_thread(&child);
            ...
            let child_control_stream = crate::kernel::control_decision::control_stream_id(&child);
            ...
            match crate::kernel::runtime_host::runtime_services::append_thread_joined_first_wins(
                &self.store,
                consumer.clone(),
                child.clone(),
...
fn fold_joined_spawn_ids(
    ...
    events
        ...
        .map(|event| {
            serde_json::from_value::<verlet_history::ThreadJoinedPayload>(event.payload.clone())
...
pub(crate) struct ChildTerminalTruth {
    pub(crate) state: verlet_history::ThreadTerminalState,
    pub(crate) outcome_reason: Option<String>,
    pub(crate) kind: verlet_history::EventKind,
    pub(crate) stream_id: verlet_history::EventStreamId,
    pub(crate) event_id: verlet_history::EventRecordId,
}

fn fold_child_terminal_truth(
    events: &[verlet_history::EventRecord],
    submitted_turn_id: &str,
) -> crate::kernel::runtime_host::VerletResult<Option<ChildTerminalTruth>> {
...
pub(crate) fn project_child_terminal_record(
    ...
    let (state, default_reason) = match event.kind {
        verlet_history::EventKind::TurnCompleted | verlet_history::EventKind::LoopCompleted => {
            (verlet_history::ThreadTerminalState::Completed, None)
        }
        ...
        verlet_history::EventKind::LoopDenied | verlet_history::EventKind::LoopBlocked => (
            verlet_history::ThreadTerminalState::Failed,
...
fn fold_orphaned_process_dispatches(
    ...
    Vec<verlet_runtime_contracts::handle::HandleDispatchEnvelope>,
> {
    ...
    {
        let witness = match serde_json::from_value::<verlet_history::IoIngressReceivedPayload>(
            event.payload.clone(),
        ) {
            Ok(witness) => witness,
            Err(err) => {
        ...
        match kind.as_str() {
            ...
            verlet_runtime_contracts::handle::HANDLE_OUTCOME_CONTENT_KIND => {
                ...
                if terminal.handle.kind != verlet_runtime_contracts::handle::HandleKind::Process {
                    continue;
                }
...
fn log_thread_skip(
    ...
    child: &verlet_runtime_contracts::ThreadCoordinates,
    stage: &str,
...
mod tests {
    ...
    impl verlet_provider::ProviderClient for RecoveryCutProvider {
        async fn complete(
            &self,
    ...
    struct RecordingProcessIngress {
        store: tokio::sync::Mutex<Option<verlet_history_sqlite::SqliteSessionStore>>,
    ...
    impl RecordingProcessIngress {
        fn with_store(store: verlet_history_sqlite::SqliteSessionStore) -> Self {
            Self {
        ...
        async fn outcomes(&self) -> Vec<verlet_runtime_contracts::handle::HandleTerminalEnvelope> {
            self.envelopes
                .lock()
                .await
                .iter()
    ...
    #[async_trait::async_trait]
    impl crate::kernel::runtime_host::runtime_api::ProcessHandleIngressSink for FailOnceProcessIngress {
        async fn submit_process_handle_envelope(
            &self,
            envelope: verlet_io_core::IngressEnvelope,
        ) -> crate::kernel::runtime_host::VerletResult<()> {
    ...
    async fn open_store(name: &str) -> verlet_history_sqlite::SqliteSessionStore {
        let root = test_root(name);
        std::fs::create_dir_all(&root).unwrap();
        verlet_history_sqlite::SqliteSessionStore::open(root.join("history.sqlite3"))
            .await
            .unwrap()
    }
    ...
    async fn seed_thread_binding(
        ...
        let claim = verlet_history::NewEventRecord::discharged(
            ...
            verlet_history::EventProvenance {
                source_streams: vec![crate::kernel::control_decision::control_stream_id(parent)],
        ...
        let mut spawned_payload = serde_json::to_value(verlet_history::ThreadSpawnedPayload {
            parent_thread_id: parent.thread_id,
            parent_turn_id: None,
        ...
        spawned.provenance = verlet_history::EventProvenance {
            source_streams: vec![crate::kernel::control_decision::control_stream_id(parent)],
            source_event_ids: vec![request_id],
            ..verlet_history::EventProvenance::default()
        };
    ...
    async fn seed_child_terminal(
        ...
        store
            .append_events(
                &verlet_history::EventStreamId::for_thread(child),
                vec![verlet_history::NewEventRecord::discharged(
                    child.clone(),
                    kind,
                    payload,
    ...
    async fn seed_process_dispatch(
        ...
        let binding = verlet_runtime_contracts::handle::HandleDispatchEnvelope {
            dispatch_id: verlet_runtime_contracts::handle::DispatchId::new(dispatch_id),
    ...
    async fn joined_payloads(
        store: &verlet_history_sqlite::SqliteSessionStore,
        parent: &verlet_runtime_contracts::ThreadCoordinates,
    ) -> Vec<(verlet_history::ThreadJoinedPayload, serde_json::Value)> {
        store
            .read_events(
                &crate::kernel::control_decision::control_stream_id(parent),
                None,
    ...
    async fn client_streams_do_not_participate_in_startup_recovery() {
        ...
        let stream_id = verlet_history::EventStreamId::new("client:orch:runs");
        store
            .append_events(
                &stream_id,
                vec![verlet_history::NewEventRecord::witnessed(
        ...
        let dispatcher = crate::kernel::process_handle_dispatch::ProcessHandleDispatcher::new(
            std::sync::Arc::new(store.clone()) as std::sync::Arc<dyn verlet_history::RuntimeStore>,
            ingress,
        );
    ...
    struct ThreadTerminalJoinCrashHost {
        ...
        fault_store: Option<
            std::sync::Arc<
                crate::support::fault::FaultingRuntimeStore<
                    verlet_history_sqlite::SqliteSessionStore,
                >,
            >,
        >,
        host: Option<crate::kernel::runtime_host::RuntimeHost>,
        parent: verlet_runtime_contracts::ThreadCoordinates,
        child: verlet_runtime_contracts::ThreadCoordinates,
    ...
    impl ThreadTerminalJoinCrashHost {
        async fn build() -> Self {
            ...
            let mut config = crate::adapters::agent_loop::AgentLoopConfig::new(
                verlet_history::ProviderApi::Other("recovery-cut".to_string()),
                "recovery-cut",
                "recovery-cut-model",
            );
            ...
            let host = crate::kernel::runtime_host::RuntimeHost::with_session_store(
                factory,
                fault_store.clone() as std::sync::Arc<dyn verlet_history::RuntimeStore>,
            );
            ...
            let dispatch = host
                ...
                .dispatch_thread_spawn(
                    ...
                    verlet_runtime_contracts::handle::DispatchId::new("provider-terminal-join-cut"),
                    "cut-child".to_string(),
    ...
    #[async_trait::async_trait]
    impl crate::support::fault_plan::CrashCutHost for ThreadTerminalJoinCrashHost {
        ...
        async fn run_to_cut(&mut self, seam: crate::support::fault_plan::CrashCutSeam) {
            ...
            let child_events = self
                .store
                .read_events(
                    &verlet_history::EventStreamId::for_thread(&self.child),
                    None,
                )
                .await
                .unwrap();
    ...
    async fn app_server_construction_runs_recovery_before_local_surfaces_are_available() {
        ...
        let store = verlet_history_sqlite::SqliteSessionStore::open(
            state_home.join("session_history.sqlite3"),
        )
        .await
        .unwrap();
        ...
        let listen = crate::adapters::app_server::AppServerListenAddr::WebSocket(
            "127.0.0.1:0".parse().unwrap(),
        );
        let mut config = crate::adapters::app_server::VerletAppServerConfig::local(listen, &root);
        ...
        let _app = crate::adapters::app_server::VerletAppServer::new_local(config)
            .await
            .unwrap();
        ...
        store
            .append_events(
                ...
                vec![verlet_history::NewEventRecord::discharged(
                    ...
                    serde_json::to_value(verlet_history::ThreadJoinedPayload {
                        child_thread_id: child.thread_id,
                        spawned_event_id,
                        terminal_state: verlet_history::ThreadTerminalState::Failed,
    ...
    async fn concurrent_live_and_recovery_join_attempts_commit_only_the_first() {
        ...
        let live = crate::kernel::runtime_host::runtime_services::append_thread_joined_first_wins(
            &store,
            parent.clone(),
            child.clone(),
            spawned_event_id,
    ...
    async fn process_orphans_fold_every_stream_and_duplicate_outcomes_deliver_once() {
        ...
        store
            .append_events(
                &crate::kernel::control_decision::control_stream_id(&legacy),
                vec![verlet_history::NewEventRecord::witnessed(
                    legacy,
                    verlet_history::EventKind::IoIngressReceived,
                    serde_json::json!({
    ...
    async fn corrupt_control_stream_does_not_block_healthy_process_recovery() {
        ...
        store
            .append_events(
                &crate::kernel::control_decision::control_stream_id(&corrupt),
                vec![verlet_history::NewEventRecord::witnessed(
                    corrupt.clone(),
                    verlet_history::EventKind::IoIngressReceived,
                    serde_json::to_value(verlet_history::IoIngressReceivedPayload {
                        ...
                        content: Some(serde_json::json!({
                            "kind": verlet_runtime_contracts::handle::HANDLE_DISPATCH_CONTENT_KIND,
    ...
    async fn malformed_process_content_isolates_only_that_item() {
        ...
        let sweep = crate::daemon::recovery_sweep::StartupRecoverySweep::new(
            store.clone(),
            crate::kernel::process_handle_dispatch::ProcessHandleDispatcher::new(
                std::sync::Arc::new(store) as std::sync::Arc<dyn verlet_history::RuntimeStore>,
                recording.clone(),
            ),
            "tenant",
            "user",
    ...
    #[async_trait::async_trait]
    impl verlet_process::live::LiveProcessBackend for BlockingProcessBackend {
    ...
    async fn process_live_in_the_dispatcher_registry_is_not_swept() {
        ...
        let dispatch_id = verlet_runtime_contracts::handle::DispatchId::new("process-live");
    ...
    fn recovery_process_envelope_reuses_the_standard_outcome_builder() {
        let binding = verlet_runtime_contracts::handle::HandleDispatchEnvelope {
            dispatch_id: verlet_runtime_contracts::handle::DispatchId::new("builder-reuse"),
            handle: verlet_runtime_contracts::handle::HandleId::process(

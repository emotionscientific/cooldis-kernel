...
pub struct AgentManifestBindOverrides {
    ...
    pub max_tool_rounds: Option<verlet_agent::manifest_schema::AgentManifestMaxToolRounds>,
    #[serde(
        default,
    ...
    pub fn from_provider_record(
        ...
        Self {
            ...
            model_ids: record
                ...
                .collect(),
            supports_streaming: verlet_provider::ProviderCapabilityRecord::for_api(
                record.api.clone(),
            )
            .supports_streaming,
        }
...
pub struct AgentManifestBoundThread {
    pub manifest: verlet_agent::manifest_schema::AgentManifestSchema,
...
fn bind_static_context_sources(
    ...
    blob_registry_root: Option<&std::path::Path>,
) -> crate::kernel::runtime_host::VerletResult<Vec<AgentManifestStaticContextSegment>> {
    let pipeline = manifest.effective_context_pipeline();
    let mut segments = Vec::new();
    for source in &pipeline.sources {
        if source.assembler != verlet_agent::manifest_schema::KERNEL_ASSEMBLER_STATIC {
            continue;
        }
        ...
        if resource.kind != verlet_agent::manifest_schema::AgentManifestResourceKind::Blob {
            continue;
        }
...
fn static_source_resource<'a>(
    input: &str,
    resources: &'a [verlet_agent::manifest_schema::AgentManifestResource],
) -> crate::kernel::runtime_host::VerletResult<
    Option<&'a verlet_agent::manifest_schema::AgentManifestResource>,
> {
...
fn missing_blob_resource_error(
    resource: &verlet_agent::manifest_schema::AgentManifestResource,
...
fn static_source_budget_share(
    pipeline: &verlet_agent::manifest_schema::AgentManifestContextPipeline,
    source_id: &str,
) -> Option<f64> {
    pipeline
        .sources
        ...
        .and_then(|source| match source.budget_share {
            Some(verlet_agent::manifest_schema::AgentManifestBudgetShare::Fraction(value)) => {
                Some(value)
            }
            Some(verlet_agent::manifest_schema::AgentManifestBudgetShare::Rest(
                verlet_agent::manifest_schema::AgentManifestBudgetRest::Rest,
            ))
            | None => None,
        })
}
...
pub struct BoundCoupling {
    ...
    pub trigger_kind: verlet_history::EventKind,
    #[serde(default, skip_serializing_if = "std::collections::BTreeMap::is_empty")]
    pub trigger_match: std::collections::BTreeMap<String, serde_json::Value>,
    pub trigger_quota: verlet_agent::manifest_schema::AgentManifestCouplingQuota,
...
fn bind_skill_resources(
    resources: &[verlet_agent::manifest_schema::AgentManifestResource],
    skill_registry_root: Option<&std::path::Path>,
) -> crate::kernel::runtime_host::VerletResult<BoundSkills> {
    let skill_resources = resources
        .iter()
        .filter(|resource| {
            resource.kind == verlet_agent::manifest_schema::AgentManifestResourceKind::Skill
        })
        .collect::<Vec<_>>();
    ...
    for resource in skill_resources {
        let parsed =
            verlet_operations::skill_package::DeclaredSkillPackageRef::parse(&resource.reference)
                .map_err(|err| {
                crate::kernel::runtime_host::VerletError::RuntimeFactory(format!(
        ...
        let record = match &parsed {
            verlet_operations::skill_package::DeclaredSkillPackageRef::Floating { name } => registry.load_record(name).map_err(|err| {
                crate::kernel::runtime_host::VerletError::RuntimeFactory(format!(
                    "skill resource {:?} floating ref {:?} was not found in the local skill registry: {err}; publish it first with `verlet skill publish <dir>`",
                    resource.name, resource.reference
                ))
            }),
            verlet_operations::skill_package::DeclaredSkillPackageRef::Pinned(reference) => registry
...
fn bind_skill_resources_from_witness(
    ...
    for resource in skill_resources {
        ...
        match verlet_operations::skill_package::DeclaredSkillPackageRef::parse(&resource.reference)?
        {
            verlet_operations::skill_package::DeclaredSkillPackageRef::Floating { name }
                if name == binding.package_name => {}
...
fn append_bound_skill(
    resource_name: &str,
    record: &verlet_operations::skill_package::PublishedSkillPackageRecord,
    ...
    context_segments.push(AgentManifestStaticContextSegment {
        ...
        assembler: verlet_agent::manifest_schema::KERNEL_ASSEMBLER_STATIC.to_string(),
...
fn discover_workspace_skills(
    ...
    resolved_path: &str,
    registry_skill_names: &std::collections::BTreeSet<String>,
) -> crate::kernel::runtime_host::VerletResult<AgentManifestSkillDiscovery> {
    ...
    for skill_dir in skill_dirs {
        ...
        let entry = verlet_operations::skill_package::SkillPackageEntry::from_skill_body(
            &skill_dir.path,
            body,
        )?;
...
) -> crate::kernel::runtime_host::VerletResult<Vec<WorkspaceSkillDirectory>> {
...
fn bind_coupling(
    coupling: &verlet_agent::manifest_schema::AgentManifestCoupling,
    operation_registry_root: Option<&std::path::Path>,
) -> crate::kernel::runtime_host::VerletResult<BoundCoupling> {
    let executor_kind =
...
fn wasm_coupling_operation_name(
    ...
    if !matches!(
        ...
        verlet_operations::operation_store::PublishedOperationSource::Wasm { .. }
    ) {
        return Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
            format!(
...
) -> crate::kernel::runtime_host::VerletResult<&'a verlet_abi::WasmOperationDefinition> {
...
fn bind_coupling_source_selector(
    coupling_id: &str,
    selector: &verlet_agent::manifest_schema::AgentManifestCouplingSelector,
) -> crate::kernel::runtime_host::VerletResult<BoundCouplingSelector> {
    let kinds = selector
        .kind
        .iter()
...
) -> crate::kernel::runtime_host::VerletResult<BoundCouplingSink> {
    let kinds = sink
        .kind
        .iter()
...
fn parse_coupling_event_kind(
    ...
    label: &str,
    value: &str,
) -> crate::kernel::runtime_host::VerletResult<verlet_history::EventKind> {
    value.parse::<verlet_history::EventKind>().map_err(|err| {
        crate::kernel::runtime_host::VerletError::RuntimeFactory(format!(
...
pub(crate) fn coupling_config_hash(
...
pub(crate) fn canonical_json_hash(
    value: &serde_json::Value,
) -> crate::kernel::runtime_host::VerletResult<String> {
...
struct SelectedManifestModelProfile<'a> {
    profile: &'a verlet_agent::manifest_schema::AgentManifestModelProfile,
    provider_id: String,
    model_id: String,
...
fn select_manifest_model_profile<'a>(
    manifest: &'a verlet_agent::manifest_schema::AgentManifestSchema,
    selection: &AgentManifestModelProfileSelection,
) -> crate::kernel::runtime_host::VerletResult<SelectedManifestModelProfile<'a>> {
...
async fn bind_protocol_tool_import(
    tool: &verlet_agent::manifest_schema::AgentManifestProtocolToolImport,
    discoverer: Option<&dyn crate::agent::tool_universe::ToolUniverseDiscoverer>,
) -> crate::kernel::runtime_host::VerletResult<crate::agent::tool_universe::ToolUniverseBinding> {
    ...
    let pin = tool
        .pin
        .as_ref()
        .map(|reference| verlet_agent::tool_ref::PinnedToolRef::parse(reference))
        .transpose()?;
    let exposes_direct = tool
        .expose
        .contains(&verlet_agent::manifest_schema::AgentManifestToolSurface::DirectTool);
...
async fn bind_operation_ref_with_expiries(
    ...
    let direct_tool_binding = direct_tool_name
        .map(|tool_name| {
            ...
            Ok::<AgentManifestDirectToolBinding, crate::kernel::runtime_host::VerletError>(
...
fn provider_id_from_ref(provider_ref: &str) -> crate::kernel::runtime_host::VerletResult<String> {
...
fn model_id_from_ref(
    ...
    if let Some((provider, model)) = id.split_once('/') {
        ...
        if model.is_empty() {
            return Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
                format!("model_ref {model_ref:?} must include a model id"),
            ));
        }
...
fn parse_operation_ref(
    operation_ref: &str,
) -> crate::kernel::runtime_host::VerletResult<OperationRef> {
    ...
    let (name, artifact_hash) = match body.split_once("@sha256:") {
        Some((name, hash)) => {
            if hash.len() != 64 || !hash.bytes().all(|byte| byte.is_ascii_hexdigit()) {
                return Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
                    format!("operation_ref {operation_ref:?} has an invalid sha256 artifact hash"),
                ));
            }
...
fn parse_operation_ref_body(
    operation_ref: &str,
    body: &str,
) -> crate::kernel::runtime_host::VerletResult<(String, Option<String>)> {
    ...
    match segments.as_slice() {
        ...
        _ => Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
            format!("operation_ref {operation_ref:?} must match {grammar}"),
        )),
    }
}

fn override_key_name(
    key: verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey,
) -> &'static str {
    match key {
        verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::DefaultCwd => "default_cwd",
        verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::Streaming => "streaming",
        verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::TurnTimeoutMs => "turn_timeout_ms",
        verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::CancellationGraceMs => "cancellation_grace_ms",
        verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::MaxToolRounds => "max_tool_rounds",
...
fn require_override_key(
    allowlist: &[verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey],
    key: verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey,
) -> crate::kernel::runtime_host::VerletResult<&'static str> {
    let name = override_key_name(key);
    if allowlist.contains(&key) {
        Ok(name)
    } else {
        Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
            format!("runtime override {name:?} is not allowlisted by the agent manifest"),
        ))
    }
}
...
fn validate_optional_positive_u64(
    ...
    if value == Some(0) {
        return Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
            format!("runtime override {label:?} must be > 0"),
        ));
    }
    Ok(())
}
...
fn validate_tool_round_budget(
    label: &str,
    value: Option<verlet_agent::manifest_schema::AgentManifestMaxToolRounds>,
) -> crate::kernel::runtime_host::VerletResult<()> {
    if value == Some(verlet_agent::manifest_schema::AgentManifestMaxToolRounds::Limited(0)) {
        return Err(crate::kernel::runtime_host::VerletError::RuntimeFactory(
            format!("runtime override {label:?} must be > 0 or \"unlimited\""),
        ));
    }
    Ok(())
}
...
pub fn resolve_manifest_placement(
    ...
    placement_override: Option<&AgentManifestPlacementBinding>,
    remote_event_store_served: bool,
) -> crate::kernel::runtime_host::VerletResult<AgentManifestPlacementBinding> {
...
fn resolve_manifest_placement_with_origin(
    ...
    placement_override: Option<&AgentManifestPlacementBinding>,
    remote_event_store_served: bool,
) -> crate::kernel::runtime_host::VerletResult<ResolvedManifestPlacement> {
...
pub struct AgentManifestWorkspaceBinding {
    ...
    pub mode: verlet_agent::manifest_schema::AgentManifestWorkspaceMode,
}
...
pub fn resolve_manifest_workspace(
    requirement: Option<&verlet_agent::manifest_schema::AgentManifestWorkspaceRequirement>,
    default_workspace: Option<&AgentManifestWorkspaceBinding>,
    workspace_override: Option<&AgentManifestWorkspaceBinding>,
) -> crate::kernel::runtime_host::VerletResult<Option<AgentManifestResolvedWorkspaceMount>> {
    Ok(
...
fn resolve_manifest_workspace_with_origin(
    ...
    default_workspace: Option<&AgentManifestWorkspaceBinding>,
    workspace_override: Option<&AgentManifestWorkspaceBinding>,
) -> crate::kernel::runtime_host::VerletResult<Option<ResolvedManifestWorkspace>> {
...
fn workspace_mode_name(
    mode: verlet_agent::manifest_schema::AgentManifestWorkspaceMode,
) -> &'static str {
    match mode {
        verlet_agent::manifest_schema::AgentManifestWorkspaceMode::ReadOnly => "ro",
        verlet_agent::manifest_schema::AgentManifestWorkspaceMode::ReadWrite => "rw",
    }
}
...
pub fn apply_runtime_overrides(
    defaults: &verlet_agent::manifest_schema::AgentManifestRuntimeDefaults,
    overrides: &AgentManifestBindOverrides,
) -> crate::kernel::runtime_host::VerletResult<(
    ...
    if let Some(value) = overrides.turn_timeout_ms {
        let key = require_override_key(
            &allowlist,
            verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::TurnTimeoutMs,
        )?;
    ...
    if let Some(value) = overrides.cancellation_grace_ms {
        let key = require_override_key(
            &allowlist,
            verlet_agent::manifest_schema::AgentManifestRuntimeOverrideKey::CancellationGraceMs,
        )?;

...
use crate::{
    DeclaredSkillPackageRef, LocalBlobRegistry, VerletError, VerletResult, validate_record_name,
};
...
use uuid::Uuid;
use verlet_agent::{validate_namespace, validate_version};
...
fn default_compatible_project_root() -> PathBuf {
    let canonical = PathBuf::from(".verlet");
    let legacy = PathBuf::from(concat!(".", "cool", "dis"));
...
impl LocalAgentRegistry {
    ...
    pub fn plan_manifest_path(&self, path: impl AsRef<Path>) -> VerletResult<AgentPublishPlan> {
    ...
    pub fn publish_manifest_path_with_operation_registry(
        &self,
        path: impl AsRef<Path>,
        operation_registry_root: impl AsRef<Path>,
    ) -> VerletResult<PublishedAgentRecord> {
        let plan = self.plan_manifest_path(path)?;
        self.publish_plan_with_operation_registry(plan, operation_registry_root)
    }

    pub fn publish_plan(&self, plan: AgentPublishPlan) -> VerletResult<PublishedAgentRecord> {
    ...
    pub fn load_record(&self, name: &str) -> VerletResult<PublishedAgentRecord> {
    ...
    pub fn load_ref(&self, agent_ref: &str) -> VerletResult<PublishedAgentRecord> {
    ...
    pub fn load_ref_with_alias_receipt(
        &self,
        agent_ref: &str,
    ) -> VerletResult<(PublishedAgentRecord, Option<AgentAliasResolutionReceipt>)> {
        let parsed = AgentRecordRef::parse(agent_ref)?;
        match parsed.version {
            Some(version) => {
                if validate_record_name(&version).is_ok() {
    ...
    pub fn list_records(&self) -> VerletResult<Vec<PublishedAgentRecord>> {
    ...
    pub fn list_version_records(&self, name: &str) -> VerletResult<Vec<AgentVersionSummary>> {
        let name = validate_record_name(name)?;
    ...
    pub fn resolve_alias(
        ...
        name: &str,
        alias: &str,
    ) -> VerletResult<(PublishedAgentRecord, AgentAliasResolutionReceipt)> {
        let name = validate_record_name(name)?;
        let alias = validate_record_name(alias)?;
    ...
    pub fn alias_record_path(&self, name: &str, alias: &str) -> VerletResult<PathBuf> {
        let name = validate_record_name(name)?;
        let alias = validate_record_name(alias)?;
        Ok(self
            .root
    ...
    pub fn version_record_path(&self, name: &str, version: &str) -> VerletResult<PathBuf> {
        let name = validate_record_name(name)?;
        validate_version(version)?;
        Ok(self
            .root
    ...
    fn write_record_atomically(&self, record: &PublishedAgentRecord) -> VerletResult<()> {
        let path = self.record_path(&record.name)?;
        write_json_atomically(&path, format!("agent record {:?}", record.name), record)
    }

    fn write_version_record_atomically(&self, record: &PublishedAgentRecord) -> VerletResult<()> {
    ...
    fn write_latest_alias_atomically(&self, record: &PublishedAgentRecord) -> VerletResult<()> {
    ...
    fn write_alias_record_atomically(&self, alias: &AgentAliasRecord) -> VerletResult<()> {
        alias.validate()?;
...
impl AgentPublishPlan {
    pub fn from_path(path: &Path) -> VerletResult<Self> {
    ...
    pub fn from_source(source: &str) -> VerletResult<Self> {
    ...
    fn from_source_with_manifest<F>(source: &str, manifest_fn: F) -> VerletResult<Self>
    where
        F: FnOnce(&toml::Value) -> VerletResult<AgentManifestSchema>,
    {
        let source_hash = text_sha256(source.as_bytes());
        let value: toml::Value = toml::from_str(source)
            .map_err(|err| VerletError::RuntimeFactory(format!("invalid agent manifest: {err}")))?;
        ...
        let namespace = manifest
            ...
            .namespace
            .clone()
            .map(|namespace| validate_namespace(&namespace))
            .transpose()?;
    ...
    pub fn verify_operation_refs(&mut self, operation_registry_root: &Path) -> VerletResult<()> {
        let mut verifications = Vec::new();
        for tool_ref in &self.tool_refs {
            if !tool_ref.reference.starts_with("op://") {
                continue;
            }
            verify_operation_ref(
                &tool_ref.name,
                &tool_ref.reference,
                &tool_ref.grants,
    ...
    fn validate_resolved_refs_for_publish(&self) -> VerletResult<()> {
...
impl PublishedAgentRecord {
    pub fn validate(&self) -> VerletResult<()> {
        ...
        validate_record_name(&self.name)?;
...
impl AgentAliasRecord {
    fn validate(&self) -> VerletResult<()> {
        ...
        validate_record_name(&self.name)?;
        validate_record_name(&self.alias)?;
        validate_version(&self.version)?;
        validate_hash_label("manifest_hash", &self.manifest_hash)?;
        Ok(())
    }
...
pub fn agent_manifest_source_from_schema(manifest: &AgentManifestSchema) -> VerletResult<String> {
    let json = serde_json::to_value(manifest).map_err(|err| {
        VerletError::RuntimeFactory(format!("failed to encode agent manifest schema: {err}"))
    })?;
    let object = json.as_object().ok_or_else(|| {
        VerletError::RuntimeFactory("agent manifest schema did not encode as an object".to_string())
    })?;
    ...
    toml::to_string_pretty(&toml::Value::Table(root)).map_err(|err| {
        VerletError::RuntimeFactory(format!("failed to encode agent manifest TOML: {err}"))
    })
}
...
fn json_to_toml_value(value: &JsonValue) -> VerletResult<Option<toml::Value>> {
    match value {
        JsonValue::Null => Ok(None),
        JsonValue::Bool(value) => Ok(Some(toml::Value::Boolean(*value))),
        ...
        JsonValue::String(value) => Ok(Some(toml::Value::String(value.clone()))),
...
impl AgentRecordRef {
    pub fn parse(value: &str) -> VerletResult<Self> {
        ...
        let name_and_version = without_scheme
            .rsplit('/')
            .next()
            .ok_or_else(|| VerletError::RuntimeFactory("empty agent ref".to_string()))?;
...
fn parse_agent_tool_refs(tools: &[AgentManifestTool]) -> VerletResult<Vec<AgentToolRef>> {
    let mut refs = Vec::new();
    for tool in tools {
        let tool_ref = match tool {
            AgentManifestTool::Bash(tool) => AgentToolRef {
                name: tool.id.clone(),
...
fn validate_tool_ref(value: &str) -> VerletResult<()> {
...
) -> VerletResult<Vec<AgentManifestResolvedRef>> {
    let mut refs = Vec::new();
    for tool in &manifest.tools {
        match tool {
            AgentManifestTool::Bash(tool) => refs.push(resolve_artifact_ref(&tool.operation_ref)),
            AgentManifestTool::Direct(tool) => refs.push(resolve_artifact_ref(&tool.operation_ref)),
...
fn prevalidate_folder_first_system_prompt(
    manifest: &AgentManifestSchema,
    prompt_path: &Path,
) -> VerletResult<FolderFirstPromptLowering> {
...
fn resolve_identity_static_source(context: &mut AgentManifestContextPipeline) -> VerletResult<()> {
    let source = context
        .sources
        .iter_mut()
        .find(|source| source.id == "identity" && source.assembler == KERNEL_ASSEMBLER_STATIC)
...
fn validate_label(label: &str, value: &str) -> VerletResult<()> {
    if value.is_empty()
        || !value
            .bytes()
...
fn canonical_json_from_schema(value: &AgentManifestSchema) -> VerletResult<JsonValue> {
    let json = serde_json::to_value(value).map_err(|err| {
        VerletError::RuntimeFactory(format!("failed to canonicalize agent manifest: {err}"))
    })?;
    Ok(sort_json(json))
}

pub(crate) fn canonical_json_from_authored_source(source: &str) -> VerletResult<JsonValue> {
    let value: toml::Value = toml::from_str(source)
        .map_err(|err| VerletError::RuntimeFactory(format!("invalid agent manifest: {err}")))?;
...
fn value_sha256(value: &JsonValue) -> VerletResult<String> {
    let bytes = serde_json::to_vec(value).map_err(|err| {
        VerletError::RuntimeFactory(format!("failed to encode resolved agent manifest: {err}"))
    })?;

use super::*;

#[test]
fn sqlite_secret_store_persists_and_redacts_status() {
    let store = SqliteSecretStore::in_memory().unwrap();

    let status = store
        .set_secret(
            "EXAMPLE_API_KEY",
            "fixture-secret",
            SecretSourceKind::Env,
            Some("EXAMPLE_API_KEY".to_string()),
        )
        .unwrap();

    assert_eq!(status.name, "EXAMPLE_API_KEY");
    assert_eq!(status.source_kind, SecretSourceKind::Env);
    assert_eq!(status.source_label.as_deref(), Some("EXAMPLE_API_KEY"));
    assert!(status.value.redacted);
    assert_eq!(store.list().unwrap()[0], status);
    let resolved = store.resolve_secret("EXAMPLE_API_KEY").unwrap().unwrap();
    assert_eq!(resolved.value, "fixture-secret");
}

#[test]
fn manifest_secret_resolution_reports_missing_refs_without_values() {
    let store = SqliteSecretStore::in_memory().unwrap();
    store
        .set_secret("VISIBLE", "fixture-secret", SecretSourceKind::Local, None)
        .unwrap();
    let manifest: WasmOperationManifest = serde_json::from_value(serde_json::json!({
        "abi": "cooldis.operation/0.1",
        "operations": [{
            "id": 1,
            "name": "search",
            "input": "json",
            "output": "json",
            "events": "none",
            "mode": "sync",
            "required_capabilities": ["secret:VISIBLE", "secret:MISSING"]
        }]
    }))
    .unwrap();

    let resolution = resolve_manifest_secret_resolution(&store, &manifest).unwrap();

    assert!(!resolution.is_ready());
    assert_eq!(
        resolution.values,
        BTreeMap::from([("VISIBLE".to_string(), "fixture-secret".to_string())])
    );
    assert_eq!(
        resolution.missing,
        std::collections::BTreeSet::from(["MISSING".to_string()])
    );
}

#[test]
fn secret_names_are_path_safe_but_allow_env_style_names() {
    assert_eq!(
        validate_secret_name("EXAMPLE_API_KEY").unwrap(),
        "EXAMPLE_API_KEY"
    );
    assert_eq!(
        validate_secret_name("provider.search-key").unwrap(),
        "provider.search-key"
    );
    assert!(validate_secret_name("../EXAMPLE_API_KEY").is_err());
    assert!(validate_secret_name("").is_err());
}

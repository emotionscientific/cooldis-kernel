...
use cooldis_runtime_contracts::{ThreadId, ThreadStatus};
...
impl CompiledThreadContract {
    ...
    pub fn contract_hash(&self) -> CooldisResult<String> {
        self.validate()?;
        let bytes = serde_json::to_vec(self).map_err(json_error)?;
        Ok(sha256_hex(&bytes))
    }

    pub fn abi_projection(&self) -> CooldisResult<AbiOperationContract> {
        self.validate()?;
...
impl ThreadContractCompiler {
    pub fn compile(source: &ThreadContractSource) -> CooldisResult<CompiledThreadContract> {
        match source.format {
...
fn compile_markdown_v0(source: &str) -> CooldisResult<CompiledThreadContract> {
    let (frontmatter, body) = parse_frontmatter(source)?;
    let name = frontmatter.get("name").cloned().ok_or_else(|| {
        CooldisError::RuntimeExecution("thread contract missing name".to_string())
    })?;
    ...
    let version = frontmatter
        ...
        .transpose()
        .map_err(|err| CooldisError::RuntimeExecution(format!("invalid thread version: {err}")))?
...
fn parse_frontmatter(source: &str) -> CooldisResult<(BTreeMap<String, String>, &str)> {
    let trimmed = source.strip_prefix("---\n").ok_or_else(|| {
        CooldisError::RuntimeExecution("agent contract missing YAML-like frontmatter".to_string())
    })?;
...
fn validate_name(label: &str, value: &str) -> CooldisResult<()> {
...
fn validate_hash(label: &str, value: &str) -> CooldisResult<()> {
...
fn json_error(err: serde_json::Error) -> CooldisError {
    CooldisError::RuntimeExecution(format!("thread contract JSON error: {err}"))
}

use crate::{
    CooldisOperationsError as CooldisError, CooldisResult, LocalBlobRegistry, LocalSkillRegistry,
    PublishedBlobRecord, PublishedSkillPackageRecord, SkillPackage, SkillPackageEntry,
...
impl SkillImportPlan {
    pub fn from_directory(skill_dir: &Path, package_name: Option<&str>) -> CooldisResult<Self> {
    ...
    pub fn artifact_hash(&self) -> CooldisResult<String> {
        Ok(wasm_sha256(&self.package.to_artifact_bytes()?))
    }

    pub fn pinned_ref(&self) -> CooldisResult<String> {
        Ok(format!(
            "skill://{}@sha256:{}",
            self.package.name,
            self.artifact_hash()?
    ...
    pub fn manifest_fragment(&self) -> CooldisResult<String> {
        let mut out = format!(
            "[[resources]]\nname = {:?}\nkind = \"skill\"\nref = {:?}\n",
            self.package.name,
            self.pinned_ref()?
    ...
    ) -> CooldisResult<PublishedSkillImport> {
...
fn relative_slash_path(root: &Path, path: &Path) -> CooldisResult<String> {
...
"#;
    ...
    fn temp_root(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!("cooldis-{label}-{}", Uuid::now_v7()))
    }

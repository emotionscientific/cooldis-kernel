...
impl LocalBlobRegistry {
    ...
    pub fn publish_file(
        ...
        name: Option<&str>,
    ) -> CooldisResult<PublishedBlobRecord> {
    ...
    pub fn load_ref(&self, ref_uri: &str) -> CooldisResult<PublishedBlobRecord> {
    ...
    pub fn load_text_ref(&self, ref_uri: &str) -> CooldisResult<(PublishedBlobRecord, String)> {
        let record = self.load_ref(ref_uri)?;
        let bytes = self.artifacts.get(&record.artifact_hash)?.ok_or_else(|| {
            CooldisError::RuntimeFactory(format!(
                "blob artifact {:?} is missing from {}; run `cooldis blob publish <file>` to publish it",
                record.ref_uri,
                self.root.display()
            ))
        })?;
    ...
    pub fn version_record_path(&self, artifact_hash: &str) -> CooldisResult<PathBuf> {
    ...
    pub fn named_record_path(&self, name: &str) -> CooldisResult<PathBuf> {
        let name = validate_record_name(name)?;
        Ok(self.root.join("names").join(format!("{name}.json")))
    }

    fn write_version_record_atomically(&self, record: &PublishedBlobRecord) -> CooldisResult<()> {
    ...
    fn write_named_record_atomically(&self, record: &PublishedBlobRecord) -> CooldisResult<()> {
...
impl BlobArtifactStore {
    ...
    fn get(&self, hash: &str) -> CooldisResult<Option<Vec<u8>>> {
...
pub fn blob_hash_from_ref(ref_uri: &str) -> CooldisResult<String> {
...
fn validate_blob_hash(hash: &str) -> CooldisResult<()> {

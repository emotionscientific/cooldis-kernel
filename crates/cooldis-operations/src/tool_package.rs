...
use cooldis_abi::{WasmOperationManifest, WasmOperationValueKind};
...
const TOOL_PACKAGE_FILE_NAME: &str = "cooldis.tool.toml";
...
impl ToolPackageSource {
    pub fn load(path: impl AsRef<Path>) -> CooldisResult<Self> {
        ...
        let summary = operation.description.clone().unwrap_or_else(|| {
            ...
            format!(
                ...
                identity
                    .description
                    .as_deref()
                    .unwrap_or("this Cooldis tool package")
            )
        });
...
fn resolve_tool_package_path(path: &Path) -> CooldisResult<PathBuf> {
...
fn validate_runtime_contract(runtime: &ToolRuntimeContract) -> CooldisResult<()> {
    match runtime.kind.as_str() {
        ...
        "kernel" => {
            if runtime.module_path.is_some() || runtime.bin_path.is_some() {
                return Err(CooldisError::RuntimeFactory(
                    "kernel tool packages are synthesized by Cooldis startup and must not declare module_path or bin_path".to_string(),
                ));
            }
            Ok(())
        }
...
fn read_json_schema(path: &Path, label: &str) -> CooldisResult<Value> {
    ...
    validate_json_schema_subset(&value, label).map_err(|err| {
        CooldisError::RuntimeFactory(format!(
            "JSON schema {} is not in the supported Cooldis schema subset: {err}",
            path.display()
        ))
    })?;
    Ok(value)
...
fn validate_fixture_json(path: &Path, schema: &Value, label: &str) -> CooldisResult<()> {
    let bytes = fs::read(path).map_err(|err| {
        CooldisError::RuntimeFactory(format!(
...
fn value_sha256(value: &impl Serialize) -> CooldisResult<String> {
    let bytes = serde_json::to_vec(value).map_err(|err| {
        CooldisError::RuntimeFactory(format!("failed to encode tool interface: {err}"))
    })?;
...
mod tests {
    ...
    use cooldis_abi::{WasmOperationDefinition, WasmOperationEventKind, WasmOperationMode};
    use serde_json::json;
  ...
  "additionalProperties": false
    ...
    fn package_source(root: &Path, fixtures: Vec<ToolFixtureDeclaration>) -> ToolPackageSource {
        ToolPackageSource {
            manifest_path: root.join("cooldis.tool.toml"),
